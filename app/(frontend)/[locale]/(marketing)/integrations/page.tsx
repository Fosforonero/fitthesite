      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-12">
        <div className="rounded-card border border-divider bg-gradient-to-br from-bg-card to-bg-secondary p-8 sm:p-12">
          ...
          <p className="mt-4 text-text-secondary max-w-2xl leading-relaxed">
            {t(
              "Qualsiasi app o smartwatch che scriva su Health Connect è automaticamente leggibile da FitMesh: Mobvoi, OnePlus Health, Zepp / Amazfit (modelli moderni), Coros, Withings (parziale), Garmin Connect (parziale), Polar Flow (parziale). Le integrazioni OAuth dedicate aggiungono dati avanzati (Training Load, Body Battery, GPS track) che Health Connect non espone.",
              "Any app or smartwatch that writes to Health Connect is automatically readable by FitMesh: Mobvoi, OnePlus Health, Zepp / Amazfit (modern models), Coros, Withings (partial), Garmin Connect (partial), Polar Flow (partial). Dedicated OAuth integrations add the advanced data (Training Load, Body Battery, GPS tracks) that Health Connect doesn't expose.",
              "Cualquier app o smartwatch que escriba en Health Connect es compatible automáticamente con FitMesh: Mobvoi, OnePlus Health, Zepp / Amazfit (modelos modernos), Coros, Withings (parcial), Garmin Connect (parcial), Polar Flow (parcial). Las integraciones OAuth dedicadas añaden datos avanzados (Training Load, Body Battery, rutas GPS) que Health Connect no expone.",
              undefined,
              undefined,
              undefined,
              "Elke app of smartwatch die naar Health Connect schrijft, is automatisch leesbaar door FitMesh: Mobvoi, OnePlus Health, Zepp / Amazfit (moderne modellen), Coros, Withings (gedeeltelijk), Garmin Connect (gedeeltelijk), Polar Flow (gedeeltelijk). Speciale OAuth-integraties voegen geavanceerde data toe (Training Load, Body Battery, GPS-tracks) die Health Connect niet blootstelt.",
              "Health Connectに書き込むアプリやスマートウォッチはFitMeshで自動的に読み取れます：Mobvoi、OnePlus Health、Zepp / Amazfit（最新モデル）、Coros、Withings（一部）、Garmin Connect（一部）、Polar Flow（一部）。専用OAuth連携はHealth Connectが公開しない高度なデータ（Training Load、Body Battery、GPSトラック）を追加します。",
              "Health Connect에 데이터를 쓰는 모든 앱이나 스마트워치는 FitMesh에서 자동으로 읽을 수 있습니다: Mobvoi, OnePlus Health, Zepp / Amazfit (최신 모델), Coros, Withings (일부), Garmin Connect (일부), Polar Flow (일부). 전용 OAuth 연동은 Health Connect가 제공하지 않는 고급 데이터(Training Load, Body Battery, GPS 트랙)를 추가합니다.",
            )}
          </p>
