...
const COPY = {
  ...
  es: {
    ...
    keyFacts: [
      ...
      { label: "Plazas fundador", value: "Primeros 1.000 cuentas: Pro de por vida gratis (activado automáticamente al registrarse)" },
  ...
  de: {
    ...
    keyFacts: [
      ...
      { label: "Gründerplätze", value: "Erste 1.000 Konten: Pro dauerhaft kostenlos (automatisch bei Registrierung)" },
  ...
  pt: {
    ...
    keyFacts: [
      ...
      { label: "Vagas de fundador", value: "Primeiras 1.000 contas: Pro vitalício grátis (ativado automaticamente no cadastro)" },
  ...
  fr: {
    ...
    keyFacts: [
      ...
      { label: "Places fondateur", value: "1 000 premiers comptes : Pro à vie gratuit (activé automatiquement à l'inscription)" },
  ...
  pl: {
    ...
    keyFacts: [
      ...
      { label: "Miejsca zalozycielskie", value: "Pierwsze 1000 kont: Pro dozywotnio za darmo (automatycznie przy rejestracji)" },
  ...
  tr: {
    ...
    keyFacts: [
      ...
      { label: "Kurucu koltuklari", value: "Ilk 1000 hesap: ömür boyu ücretsiz Pro (kayitta otomatik)" },
  ...
  ja: {
    ...
    blurb50: {
      title: "50語",
      body:
        "FitMesh Syncは、Galaxy Watch、Mi Band、Polar、Garmin、Fitbitなどのウェアラブルの健康データを1つのプレミアムダッシュボードに統合するAndroid・iOSアプリです。プライバシーファースト、EUサーバー、イタリア開発。トラッカーやデータブローカーなしでHealth Connect(Android)とApple ヘルスケア(iOS)を介して動作します。2026年6月よりGoogle PlayとApp Storeで提供中。iOS版はEU圏外の対応ストアで配信中、EUでの展開は進行中。",
    },
    blurb100: {
      title: "100語",
      body:
        "FitMesh Syncは、市場のあらゆるスマートウォッチやフィットネスバンドからの健康データを1つのプレミアムダッシュボードに統合するため、Matteo Pizzi（Fosforonero）がイタリアで開発したAndroid・iOSアプリです。Androidでは、Health Connectの保存先として動作するため、Galaxy Watch、Mi Band、Polar、Garmin、Fitbit、Withings、Honor、Huawei、OuraとブランドごとのOAuthなしで標準対応します。iOSでは、Apple ヘルスケアから直接読み取り、BluetoothでColmiリングに接続します。プライバシーファースト：EUサーバー、GDPR、データブローカーなし。2026年6月よりGoogle PlayとApp Storeで提供中(iOS版はEU圏外の対応ストアで配信中、EU展開も進行中)。家族と健康データを共有するMesh Family機能も近日登場。",
    },
    blurb200: {
      title: "200語（フルプロフィール）",
      body:
        "FitMesh Syncは2026年、Galaxy Watchを使い、妻はMi Band、母はWithingsを使う開発者（Matteo Pizzi、Fosforoneroスタジオ）の不満からイタリアで生まれました。各ブランドはデータを自社アプリ内に閉じ込めます。誰も統合されたプレミアムなビューを提供していません。FitMeshはまさにこれを解決します。Health Connect（2024年以降、ほぼすべてのウェアラブルのデータを集めるAndroidの標準）から読み取り、歩数・心拍数・睡眠・カロリー・ワークアウトなどの指標を、システム管理者向けではなく日々の閲覧のために設計されたネイティブFlutterダッシュボードに集約します。最初のコミットからプライバシーファースト：欧州サーバー（Supabaseフランクフルト）、真のGDPR準拠、データブローカーなし、トラッカーなし。同カテゴリのクラウド間ブリッジアプリとの戦略的な違いは、FitMeshが静かなデータルーターではなく「保存先」である点です。データは保存され、丁寧なグラフで可視化され、同じ家族のメンバー間で共有できます（Mesh Family）：それぞれが自分のデータを他のメンバーと共有し、ひとつのダッシュボードにまとまります。アプリはGoogle PlayとApp Storeで提供中：最初の1,000人のファウンダーは登録時に自動で生涯Proが無料になります。iOS版はEU圏外の対応ストアで配信中で、EUでの展開も進行中です。",
    },

    keyFactsTitle: "主要データ",
    keyFacts: [
      { label: "ローンチ", value: "2026年6月よりGoogle PlayとApp Storeで公開 · iOSはEU圏外で配信中、EU展開は進行中" },
      { label: "プラットフォーム", value: "Android + iOS（配信中、EU展開は近日）" },
  ...
  ko: {
    ...
    blurb50: {
      title: "50단어",
      body:
        "FitMesh Sync는 Galaxy Watch, Mi Band, Polar, Garmin, Fitbit 등 웨어러블의 건강 데이터를 하나의 프리미엄 대시보드로 통합하는 Android 및 iOS 앱입니다. 프라이버시 우선, EU 서버, 이탈리아에서 개발. 트래커나 데이터 브로커 없이 Health Connect(Android)와 Apple 건강(iOS)을 통해 작동합니다. 2026년 6월부터 Google Play와 App Store에서 이용 가능; iOS 앱은 EU 외 지원되는 스토어에서 이미 출시되었으며, EU 배포는 진행 중입니다.",
    },
    blurb100: {
      title: "100단어",
      body:
        "FitMesh Sync는 시장의 모든 스마트워치와 피트니스 밴드의 건강 데이터를 하나의 프리미엄 대시보드로 통합하기 위해 Matteo Pizzi(Fosforonero)가 이탈리아에서 개발한 Android 및 iOS 앱입니다. Android에서는 Health Connect 대상으로 작동하므로 Galaxy Watch, Mi Band, Polar, Garmin, Fitbit, Withings, Honor, Huawei, Oura와 브랜드별 OAuth 없이 기본 호환되며, iOS에서는 Apple 건강에서 직접 읽고 블루투스로 Colmi 링에 연결됩니다. 프라이버시 우선: EU 서버, GDPR, 데이터 브로커 없음. 2026년 6월부터 Google Play와 App Store에서 이용 가능(iOS 앱은 EU 외 지원되는 스토어에서 이미 출시, EU 배포 진행 중); 가족과 건강 데이터를 공유하는 Mesh Family 기능이 곧 출시됩니다.",
    },
    blurb200: {
      title: "200단어 (전체 프로필)",
      body:
        "FitMesh Sync는 Galaxy Watch를 쓰고, 아내는 Mi Band, 어머니는 Withings를 사용하는 한 개발자(Matteo Pizzi, Fosforonero 스튜디오)의 불만에서 2026년 이탈리아에서 탄생했습니다. 각 브랜드는 자사 앱 안에 데이터를 가둡니다. 통합된 프리미엄 뷰를 제공하는 곳은 없습니다. FitMesh는 바로 이 문제를 해결합니다. Health Connect(2024년부터 사실상 모든 웨어러블의 데이터를 모으는 Android 표준)에서 읽어, 걸음 수·심박수·수면·칼로리·운동 등의 지표를 시스템 관리자가 아닌 일상적인 열람을 위해 설계된 네이티브 Flutter 대시보드로 집계합니다. 첫 커밋부터 프라이버시 우선: 유럽 서버(Supabase 프랑크푸르트), 실질적인 GDPR 준수, 데이터 브로커 없음, 트래커 없음. 동종 클라우드 간 브리지 앱과의 전략적 차이는 FitMesh가 조용한 데이터 라우터가 아니라 목적지라는 점입니다. 데이터는 저장되고, 정성스러운 차트로 시각화되며, 같은 가족 구성원 간에 공유할 수 있습니다(Mesh Family): 각자 자신의 데이터를 다른 구성원과 공유하며, 하나의 대시보드에 모입니다. 앱은 Google Play와 App Store에서 이용 가능합니다: 처음 1,000명의 파운더는 가입 시 자동으로 평생 Pro를 무료로 받습니다. iOS 앱은 EU 외 지원되는 스토어에서 이미 출시되었으며, EU 배포는 진행 중입니다.",
    },

    keyFactsTitle: "핵심 정보",
    keyFacts: [
      { label: "출시", value: "2026년 6월부터 Google Play와 App Store에 공개 · iOS EU 외 출시, EU 배포 진행 중" },
      { label: "플랫폼", value: "Android + iOS (출시됨; EU 배포 진행 중)" },
