...
async function gestisci(req: Request): Promise<Response> {
  ...
  // ── La traccia del tentativo ────────────────────────────────────────────
  //
  // Si scrive QUI: dopo l'autenticazione e dopo che il payload e'
  // sintatticamente valido, e PRIMA di qualunque giudizio sul prodotto.
  //
  // Nella route viva stava piu' in basso, e quattro ritorni la precedevano —
  // fra cui `unknown_product`, che e' esattamente la forma che assumerebbe un
  // disallineamento con App Store Connect. Il commento della route diceva gia'
  // «si scrive PRIMA della validazione di proposito»: questa e' la posizione
  // che quel commento descriveva.
  //
  // NON PUO' CAMBIARE L'ESITO DELLA RICHIESTA. L'unico modo in cui puo'
  // fallire e' non lasciando la traccia; non deve mai essere il motivo per cui
  // un pagamento non va a buon fine. Per questo l'errore si registra e si
  // prosegue, invece di propagarlo.
  //
  ...
  // puo' scrivere tentativi a nome di altri. Non contiene ne' token ne'
  // ricevute: soltanto chi, quando, quale piattaforma e quale prodotto.
  try {
  ...
  if (platform === "ios") {
    ...
    try {
      ...
      console.warn(
        `[Billing] apple_validation_failed status=${result.status}`,
      );
  ...
  try {
    if (isSubscription) {
      ...
      if (result.kind === "error") {
        console.warn(
          `[Billing] google_validation_failed sub status=${result.status}`,
        );
    ...
    if (result.kind === "error") {
      console.warn(
        `[Billing] google_validation_failed product status=${result.status}`,
      );
      return jsonError(502, "google_validation_failed", { status: result.status });
    }
    return jsonError(500, "unexpected_result_kind");
