/**
 * POST /api/v1/oauth/dexcom/exchange — token exchange server-side.
 *
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const resp = await exchangeDexcomCode(code, redirect_uri, code_verifier);
    return jsonOk({
    ...
    if (err instanceof DexcomApiError) {
    ...
    console.error("[dexcom/exchange] unexpected error:", err);
