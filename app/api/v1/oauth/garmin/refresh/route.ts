/**
 * POST /api/v1/oauth/garmin/refresh — refresh token OAuth2 server-side.
 *
 * SCAFFOLD. Body (JSON): { refresh_token: string }
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const token = await refreshGarminToken(parsed.data.refresh_token);
    return jsonOk({
    ...
    console.error("[garmin/refresh] unexpected error:", err);
