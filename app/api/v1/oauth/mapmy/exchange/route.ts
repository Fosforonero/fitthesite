/**
 * POST /api/v1/oauth/mapmy/exchange — token exchange server-side.
 *
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const resp = await exchangeMapMyCode(code, redirect_uri, code_verifier);
    return jsonOk({
    ...
    if (err instanceof MapMyApiError) {
    ...
    console.error("[mapmy/exchange] unexpected error:", err);
