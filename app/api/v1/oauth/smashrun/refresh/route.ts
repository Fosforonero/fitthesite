/**
 * POST /api/v1/oauth/smashrun/refresh — token refresh server-side.
 *
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const resp = await refreshSmashrunToken(refresh_token);
    return jsonOk({
    ...
    console.error("[smashrun/refresh] unexpected error:", err);
