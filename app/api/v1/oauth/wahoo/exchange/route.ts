/**
 * POST /api/v1/oauth/wahoo/exchange — token exchange server-side.
 *
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const resp = await exchangeWahooCode(code, redirect_uri, code_verifier);
    return jsonOk({
    ...
    console.error("[wahoo/exchange] unexpected error:", err);
