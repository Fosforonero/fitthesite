...
- **FitMesh**: Disponibile su Android e iOS. Architettura articolata distintamente tra:
  1. *Bridge sul dispositivo*: componente di sincronizzazione locale e lettura/scrittura on-device (Health Connect su Android, Apple Health su iOS). Nessun claim di «bassa latenza» viene attribuito finché non supportato da benchmark empirici misurati.
  2. *App mobile*: interfaccia utente su smartphone per controllo dello stato, associazione dispositivi e diagnostica.
  3. *Backend reale*: infrastruttura server/API FitMesh deputata alla gestione di identità/autenticazione, sessioni, sincronizzazione impostazioni e stato abbonamenti/licenze. Rispetto ai dati salute dell'utente, il ruolo effettivo del backend non è quello di un database cloud centralizzato delle metriche (l'archiviazione e l'aggregazione primaria avvengono localmente sul dispositivo tramite Health Connect e Apple Health per preservare privacy e controllo locale). Il backend reale non viene definito né trattato come "opzionale" finché l'analisi del codice di produzione non ne dimostri l'effettiva facoltatività.
