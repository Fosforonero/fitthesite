...
## 3. Stato Piattaforme Web / Browser per la Fase 2 (Schede Rettificate)

- **FitMesh**: Disponibile su Android e iOS. Architettura articolata distintamente tra:
  1. *Bridge e acquisizione sul dispositivo*: componente di sincronizzazione e interfaccia locale on-device con i framework salute di sistema (Health Connect su Android, Apple Salute su iOS) e con eventuali sorgenti hardware collegate direttamente. Nessun claim di «bassa latenza» viene attribuito finché non supportato da benchmark empirici misurati.
  2. *App mobile*: interfaccia utente su smartphone per controllo dello stato della sincronizzazione, associazione e configurazione dei dispositivi e diagnostica.
  3. *Backend reale*: infrastruttura server/API FitMesh. Il funzionamento effettivo prevede due livelli reali distinti: la lettura dai sistemi salute presenti sullo smartphone e il successivo invio/conservazione delle metriche sincronizzate nel backend FitMesh, nei limiti documentati dal codice (es. endpoint `/api/v1/sync`, isolamento delle tabelle utente via RLS) e dalla Privacy Policy dell'applicazione. Il backend reale non viene definito né trattato come "opzionale" e non viene promesso alcun trattamento esclusivamente locale dei dati salute.
- **Health Sync**: Soluzione di sincronizzazione gestita localmente sul dispositivo secondo la documentazione ufficiale del produttore, con connettori dedicati e gestione dei permessi on-device tra le sorgenti e le destinazioni configurate dall'utente.
- **Oura on the Web**: Servizio web per la consultazione e visualizzazione dei dati biometrici accessibile oggi via browser, con chiusura definitiva della dashboard annunciata ufficialmente da Oura per il 5 ottobre 2026. L'accesso browser rimarrà disponibile tramite il Membership Hub per la gestione dell'abbonamento/account e per l'esportazione documentata dei propri archivi storici di dati personali. Non vengono promessi token personali, endpoint specifici o formati di esportazione non comprovati da documentazione ufficiale.
