...
export async function validateAppleReceipt(args: {
  ...
  if (r.httpStatus !== 200 || !r.data) {
    // Il corpo grezzo della risposta Apple NON esce da qui.
    //
    // Prima era `r.raw.slice(0, 300)`, e finiva in un `console.warn` della
    // route. Trecento caratteri della risposta a una verifica di ricevuta
    // possono contenere frammenti della ricevuta stessa, e un log e' un posto
    // in cui non si sceglie chi guarda.
    //
    // Quello che resta e' tutto cio' che serve davvero a indagare: quale
    // ambiente, quale stato HTTP. Il contenuto non lo si potrebbe comunque
    // scrivere da nessuna parte, quindi portarlo in giro e' solo il rischio
    // senza il beneficio.
    return {
      kind: "error",
      status: r.httpStatus,
      body: `http_${r.httpStatus}_${environment}`,
    };
  }
  if (r.data.status !== 0) {
