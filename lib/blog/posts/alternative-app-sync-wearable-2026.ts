...
export const post: BlogPost = {
  ...
  body: [
    ...
    },
    {
      ...
      items: {
        it: [
          "**FitMesh Sync**: legge da Health Connect su Android e da Apple Salute su iPhone, con OAuth già attivo per Garmin, Polar e Withings (Oura in arrivo). Strava è ad accesso limitato: la lettura funziona per chi è già connesso, nuove connessioni richiedono l'approvazione di Strava. Pricing one-shot.",
        ...
        en: [
          "**FitMesh Sync**: reads from Health Connect on Android and Apple Health on iPhone, with live OAuth already active for Garmin, Polar and Withings (Oura coming soon). Strava has limited access: reading works for accounts already connected, new connections require Strava's approval. One-shot pricing.",
        ...
        es: [
          "**FitMesh Sync**: lee desde Health Connect en Android y Apple Salud en iPhone, con OAuth ya activo para Garmin, Polar y Withings (Oura próximamente). Strava tiene acceso limitado: la lectura funciona para quienes ya están conectados, las nuevas conexiones requieren la aprobación de Strava. Pago único.",
        ...
        de: [
          "**FitMesh Sync**: liest aus Health Connect auf Android und Apple Health auf iPhone, mit bereits aktivem OAuth für Garmin, Polar und Withings (Oura kommt bald). Strava hat eingeschränkten Zugang: Das Lesen funktioniert für bereits verbundene Konten, neue Verbindungen bedürfen der Freigabe durch Strava. Einmaliger Kaufpreis.",
        ...
        pt: [
          "**FitMesh Sync**: lê do Health Connect no Android e do Apple Saúde no iPhone, com OAuth já ativo para Garmin, Polar e Withings (Oura em breve). O Strava tem acesso limitado: a leitura funciona para quem já está conectado, novas conexões dependem da aprovação da Strava. Compra única.",
        ...
        fr: [
          "**FitMesh Sync**: lit depuis Health Connect sur Android et Apple Santé sur iPhone, avec une intégration OAuth déjà active pour Garmin, Polar et Withings (Oura arrive bientôt). Strava a un accès limité : la lecture fonctionne pour les comptes déjà connectés, les nouvelles connexions nécessitent l'approbation de Strava. Achat unique.",
        ...
        pl: [
          "**FitMesh Sync**: odczytuje z Health Connect na Androidzie i Apple Zdrowie na iPhonie, z już aktywnym OAuth dla Garmin, Polar i Withings (Oura wkrótce). Strava ma ograniczony dostęp: odczyt działa dla już połączonych kont, nowe połączenia wymagają zatwierdzenia przez Stravę. Jednorazowa płatność.",
        ...
        tr: [
          "**FitMesh Sync**: Android'de Health Connect'ten, iPhone'da Apple Sağlık'tan okur; Garmin, Polar ve Withings için OAuth zaten aktif (Oura yakında). Strava'da erişim sınırlıdır: okuma zaten bağlı hesaplar için çalışır, yeni bağlantılar Strava'nın onayına tabidir. Tek seferlik fiyatlandırma.",
        ...
        nl: [
          "**FitMesh Sync**: leest van Health Connect op Android en Apple Gezondheid op iPhone, met al actieve OAuth voor Garmin, Polar en Withings (Oura komt binnenkort). Strava heeft beperkte toegang: lezen werkt voor reeds gekoppelde accounts, nieuwe koppelingen vereisen goedkeuring van Strava. Eenmalige prijs.",
        ...
        ja: [
          "**FitMesh Sync**: AndroidではHealth Connect、iPhoneではApple ヘルスケアから読み取り、Garmin、Polar、WithingsのOAuthはすでに有効（Ouraは近日公開）。Stravaはアクセスが限定的です：読み取りはすでに接続済みのアカウントで機能しますが、新規接続にはStravaの承認が必要です。買い切り価格。",
        ...
        ko: [
          "**FitMesh Sync**: Android에서는 Health Connect, iPhone에서는 Apple 건강 앱에서 읽어옵니다. Garmin, Polar, Withings OAuth는 이미 활성화되어 있습니다(Oura는 출시 예정). Strava는 접근이 제한적입니다: 이미 연결된 계정은 읽기가 작동하지만, 새로운 연결은 Strava의 승인이 필요합니다. 일회성 가격.",
    ...
    },
    {
      ...
      items: {
        it: [
          ...
          "**Vuoi una dashboard pulita nell'app per Galaxy/Pixel/Fitbit/Mi Band**: prova FitMesh Sync (privacy-first, one-time payment).",
        ...
        en: [
          ...
          "**Want a clean dashboard in the app for Galaxy/Pixel/Fitbit/Mi Band**: try FitMesh Sync (privacy-first, one-time payment).",
        ...
        es: [
          ...
          "**Quieres un panel limpio en la app para Galaxy/Pixel/Fitbit/Mi Band**: prueba FitMesh Sync (privacidad ante todo, pago único).",
        ...
        de: [
          ...
          "**Du möchtest ein übersichtliches Dashboard in der App für Galaxy/Pixel/Fitbit/Mi Band**: Probiere FitMesh Sync (Datenschutz zuerst, Einmalkauf).",
        ...
        pt: [
          ...
          "**Quer um painel limpo no app para Galaxy/Pixel/Fitbit/Mi Band**: experimente o FitMesh Sync (privacidade em primeiro lugar, compra única).",
        ...
        fr: [
          ...
          "**Vous voulez un tableau de bord épuré dans l'application pour Galaxy/Pixel/Fitbit/Mi Band**: essayez FitMesh Sync (confidentialité avant tout, achat unique).",
        ...
        pl: [
          ...
          "**Chcesz przejrzystego panelu w aplikacji dla Galaxy/Pixel/Fitbit/Mi Band**: wypróbuj FitMesh Sync (prywatność na pierwszym miejscu, jednorazowa płatność).",
        ...
        tr: [
          ...
          "**Galaxy/Pixel/Fitbit/Mi Band için uygulamada temiz bir panel istiyorsanız**: FitMesh Sync'i deneyin (gizlilik önce, tek seferlik ödeme).",
        ...
        nl: [
          ...
          "**Een overzichtelijk dashboard in de app voor Galaxy/Pixel/Fitbit/Mi Band**: probeer FitMesh Sync (privacy-first, eenmalige betaling).",
        ...
        ja: [
          ...
          "**Galaxy/Pixel/Fitbit/Mi Bandのためのアプリ内のクリーンなダッシュボードが欲しい**：FitMesh Syncを試してください（プライバシー優先、買い切り）。",
        ...
        ko: [
          ...
          "**Galaxy/Pixel/Fitbit/Mi Band를 위한 앱 내 깔끔한 대시보드를 원하는 경우**: FitMesh Sync를 시도해보세요 (프라이버시 우선, 일회성 결제).",
    ...
    },
    {
      type: "cta",
      title: {
        it: "Vuoi provare la nostra dashboard unificata nell'app?",
        en: "Want to try our unified dashboard in the app?",
        es: "¿Quieres probar nuestro panel unificado en la app?",
        de: "Möchtest du unser einheitliches Dashboard in der App ausprobieren?",
        pt: "Quer experimentar nosso painel unificado no app?",
        fr: "Vous souhaitez essayer notre tableau de bord unifié dans l'application?",
        pl: "Chcesz wypróbować nasz ujednolicony panel w aplikacji?",
        tr: "Uygulamada birleşik panelimizi denemek ister misiniz?",
        nl: "Wil je ons uniforme dashboard in de app uitproberen?",
        ja: "アプリで統合ダッシュボードを試してみませんか？",
        ko: "앱에서 통합 대시보드를 사용해보고 싶으신가요?",
      },
      body: {
        ...
        es: "FitMesh Sync se descarga gratis para todos; los primeros 1000 founders (hasta el 31 de julio de 2026) recibieron además el Pro de por vida gratis. Funciona hoy con todos los wearables que escriben en Health Connect.",
        de: "FitMesh Sync steht für alle kostenlos zum Download; die ersten 1000 Founder (bis 31. Juli 2026) haben zusätzlich lebenslanges Pro gratis erhalten. Funktioniert heute mit allen Wearables, die in Health Connect schreiben.",
        pt: "FitMesh Sync é gratuito para baixar por qualquer pessoa; os primeiros 1000 founders (até 31 de julho de 2026) receberam também o Pro vitalício grátis. Funciona hoje com todos os wearables que escrevem no Health Connect.",
        fr: "FitMesh Sync est gratuit au téléchargement pour tout le monde ; les 1000 premiers founders (jusqu'au 31 juillet 2026) ont reçu en plus le Pro à vie gratuit. Fonctionne dès aujourd'hui avec tous les appareils connectés qui écrivent dans Health Connect.",
        pl: "FitMesh Sync jest bezpłatny do pobrania dla każdego; pierwsi 1000 founderów (do 31 lipca 2026) otrzymało dodatkowo dożywotnie Pro za darmo. Działa dziś ze wszystkimi urządzeniami wearable zapisującymi do Health Connect.",
        tr: "FitMesh Sync herkes için ücretsiz indirilebilir; ilk 1000 kurucu (31 Temmuz 2026'ya kadar) ayrıca ömür boyu ücretsiz Pro kazandı. Bugün Health Connect'e yazan tüm giyilebilirlerle çalışır.",
        nl: "FitMesh Sync is voor iedereen gratis te downloaden; de eerste 1000 founders (tot 31 juli 2026) hebben ook levenslang Pro gratis gekregen. Werkt vandaag met alle wearables die naar Health Connect schrijven.",
        ja: "FitMesh Syncは誰でも無料でダウンロードでき、最初の1000名のfounder（2026年7月31日まで）にはさらに永久版Proも無料で付与されました。今日Health Connectに書き込むすべてのウェアラブルで動作します。",
        ko: "FitMesh Sync는 누구나 무료로 다운로드할 수 있으며, 처음 1000명의 founder(2026년 7월 31일까지)에게는 평생 Pro도 무료로 제공되었습니다. 오늘날 Health Connect에 쓰는 모든 웨어러블과 작동합니다.",
      },
      ctaLabel: {
      ...
      ctaHref: { it: "/it#download", en: "/en#download", pl: "/pl#download", tr: "/tr#download", nl: "/nl#download", ja: "/ja#download", ko: "/ko#download" },
    },
  ],
  faq: [
    {
    ...
    },
    {
      ...
      a: {
        it: "Solo in parte. FitMesh Sync è una dashboard unificata nell'app: legge da varie source e mostra tutto in un posto. Non fa sync background tra app terze (es. Samsung Health → Fitbit). Per quello scenario specifico le bridge app verticali restano più adatte. Se invece ti basta vedere i dati in un posto pulito, FitMesh è più semplice.",
        en: "Partially. FitMesh Sync is a unified dashboard in the app: reads from various sources and shows everything in one place. It doesn't do background sync between third-party apps (e.g. Samsung Health → Fitbit). For that specific scenario vertical bridge apps remain better suited. If you just want to see data in one clean place, FitMesh is simpler.",
        es: "Solo en parte. FitMesh Sync es un panel unificado en la app: lee desde varias fuentes y muestra todo en un único lugar. No realiza sincronización en segundo plano entre apps de terceros (por ejemplo, Samsung Health → Fitbit). Para ese escenario concreto, las apps puente verticales siguen siendo más adecuadas. Si lo que necesitas es ver tus datos en un lugar limpio y organizado, FitMesh es la opción más sencilla.",
        de: "Nur teilweise. FitMesh Sync ist ein einheitliches Dashboard in der App: Es liest aus verschiedenen Quellen und zeigt alles an einem Ort an. Es macht keine Hintergrund-Synchronisierung zwischen Drittanbieter-Apps (z. B. Samsung Health → Fitbit). Für dieses spezifische Szenario sind vertikale Bridge-Apps besser geeignet. Wenn du einfach nur Daten an einem übersichtlichen Ort sehen möchtest, ist FitMesh die einfachere Wahl.",
        pt: "Apenas parcialmente. FitMesh Sync é um painel unificado no app: lê de várias fontes e mostra tudo em um único lugar. Ele não faz sincronização em segundo plano entre apps de terceiros (por exemplo, Samsung Health → Fitbit). Para esse cenário específico, os apps ponte verticais continuam sendo mais adequados. Se você só precisa ver os dados em um lugar limpo, o FitMesh é mais simples.",
        fr: "Partiellement. FitMesh Sync est un tableau de bord unifié dans l'application: il lit depuis diverses sources et affiche tout en un seul endroit. Il ne synchronise pas en arrière-plan entre applications tierces (par exemple Samsung Health → Fitbit). Pour ce scénario spécifique, les applications pont verticales restent mieux adaptées. Si vous souhaitez simplement voir vos données en un endroit épuré, FitMesh est plus simple.",
        pl: "Tylko w części. FitMesh Sync to jedna unifikowana dashboard w aplikacji: czyta z różnych źródeł i pokazuje wszystko w jednym miejscu. Nie wykonuje synchronizacji tła między aplikacjami trzecich stron (np. Samsung Health → Fitbit). W tym konkretnym scenariuszu, bridge aplikacyjne poziome nadal są bardziej odpowiednie. Jeśli chcesz tylko zobaczyć dane w jednym czystym miejscu, FitMesh jest prościej.",
        tr: "Sadece kısmen. FitMesh Sync birUnified dashboard uygulamada: çeşitli kaynaklardan oku ve tüm bilgileri tek bir yerde gösterir. Üçüncü taraf uygulamaları (örn. Samsung Health → Fitbit) arası arka planda sync yapmaz. Bu özel senaryo için vertical bridge uygulamaları daha uygun kalır. Eğer sadece temiz bir yerde verileri görmek istiyorsanız, FitMesh daha basit.",
        nl: "Gedeeltelijk. FitMesh Sync is een uniform dashboard in de app: leest van verschillende bronnen en toont alles op één plek. Het doet geen synchronisatie op de achtergrond tussen apps van derden (bijv. Samsung Health → Fitbit). Voor dat specifieke scenario blijven verticale brug-apps beter geschikt. Als je alleen gegevens op één overzichtelijke plek wil zien, is FitMesh eenvoudiger.",
        ja: "部分的にのみです。FitMesh Syncはアプリ内の統合ダッシュボードです：様々なソースから読み取り、すべてを1か所に表示します。サードパーティアプリ間のバックグラウンド同期（例：Samsung Health → Fitbit）は行いません。その特定のシナリオでは、垂直ブリッジアプリの方が適しています。1か所でデータをきれいに見たいだけであれば、FitMeshはより簡単です。",
        ko: "부분적으로만입니다. FitMesh Sync는 앱 내 통합 대시보드입니다: 다양한 소스에서 읽어 모든 것을 한 곳에 표시합니다. 서드파티 앱 간의 백그라운드 동기화(예: Samsung Health → Fitbit)는 하지 않습니다. 그 특정 시나리오에서는 수직 브리지 앱이 더 적합합니다. 한 곳에서 데이터를 깔끔하게 보고 싶다면 FitMesh가 더 간단합니다.",
      },
    },
  ],
  related: [
