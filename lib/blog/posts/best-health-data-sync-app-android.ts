  ...
  body: [
    ...
    },
    {
      ...
      text: {
        ...
        de: "FitMesh Sync ist eine der Android-Apps, die nativ aus Health Connect liest: Sie empfängt Daten von Galaxy Watch, Garmin und jedem Wearable, das in Health Connect schreibt, aggregiert sie ohne Duplikate und zeigt sie in einem über den Browser zugänglichen Web-Dashboard an. Es ist nicht die einzige Option in dieser Kategorie, aber ein gutes Beispiel dafür, wie eine Sync-App technisch funktionieren sollte: kein dauerhaft eingeschaltetes Bluetooth, kein Scraping, keine proprietären APIs.",
        pt: "FitMesh Sync é um dos apps Android que lê do Health Connect de forma nativa: recebe dados do Galaxy Watch, Garmin e qualquer wearable que grave no Health Connect, agrega sem duplicatas e exibe em um painel web acessível pelo navegador. Não é a única opção nessa categoria, mas é um bom exemplo de como um app de sincronização deve funcionar tecnicamente: sem Bluetooth sempre ativo, sem scraping, sem APIs proprietárias.",
        fr: "FitMesh Sync est l'une des applications Android qui lit depuis Health Connect nativement: elle reçoit des données de Galaxy Watch, Garmin et tout wearable qui écrit dans Health Connect, les agrège sans doublons et les affiche dans un tableau de bord web accessible depuis n'importe quel navigateur. Ce n'est pas la seule option dans cette catégorie, mais c'est un bon exemple de la façon dont une application de synchronisation devrait fonctionner techniquement: pas de Bluetooth toujours actif, pas de scraping, pas d'API propriétaires.",
        pl: "FitMesh Sync to jedna z aplikacji na Androida, która odczytuje dane z Health Connect natywnie: odbiera dane z Galaxy Watch, Garmin i każdego wearable'a zapisującego w Health Connect, agreguje je bez duplikatów i wyświetla w dashboardzie webowym dostępnym z przeglądarki. To nie jedyna opcja w tej kategorii, ale dobry przykład tego, jak aplikacja do synchronizacji powinna działać technicznie: żadnego stale włączonego Bluetooth, żadnego scrapingu, żadnych zastrzeżonych API.",
        tr: "FitMesh Sync, Health Connect'ten yerel olarak okuyan Android uygulamalarından biridir: Galaxy Watch, Garmin ve Health Connect'e yazan her giyilebilir cihazdan veri alır, bunları yinelemesiz toplar ve tarayıcıdan erişilebilen bir web gösterge panelinde gösterir. Bu kategorideki tek seçenek değildir, ancak bir senkronizasyon uygulamasının teknik olarak nasıl çalışması gerektiğinin iyi bir örneğidir: her zaman açık Bluetooth yok, scraping yok, tescilli API yok.",
        nl: "FitMesh Sync is een van de Android-apps die native uit Health Connect leest: het ontvangt gegevens van Galaxy Watch, Garmin en elk wearable dat naar Health Connect schrijft, aggregeert ze zonder duplicaten en toont ze in een via de browser toegankelijk webdashboard. Het is niet de enige optie in deze categorie, maar het is een goed voorbeeld van hoe een sync-app technisch zou moeten werken: geen altijd ingeschakeld Bluetooth, geen scraping, geen eigen API's.",
        ja: "FitMesh SyncはHealth Connectからネイティブに読み取るAndroidアプリの一つです。Galaxy Watch、Garminや、Health Connectに書き込む任意のウェアラブルからデータを受け取り、重複なく集約し、ブラウザからアクセスできるWebダッシュボードに表示します。このカテゴリの唯一の選択肢ではありませんが、同期アプリが技術的にどのように機能すべきかの良い例です。常時オンのBluetoothなし、スクレイピングなし、独自APIなし。",
        ko: "FitMesh Sync는 Health Connect에서 기본적으로 읽는 Android 앱 중 하나입니다. Galaxy Watch, Garmin 및 Health Connect에 쓰는 모든 웨어러블에서 데이터를 수신하고, 중복 없이 집계하여 브라우저에서 접근 가능한 웹 대시보드에 표시합니다. 이 범주에서 유일한 선택지는 아니지만, 동기화 앱이 기술적으로 어떻게 작동해야 하는지의 좋은 예입니다. 항상 켜진 블루투스 없음, 스크래핑 없음, 독점 API 없음.",
      },
    },
    {
      type: "heading",
    ...
    },
    {
      ...
      body: {
        it: "FitMesh Sync si collega a Health Connect direttamente, aggrega dati da Galaxy Watch, Garmin e altri wearable senza duplicati, e li mostra in una dashboard web accessibile da qualsiasi browser.",
        en: "FitMesh Sync connects to Health Connect directly, aggregates data from Galaxy Watch, Garmin, and other wearables without duplicates, and displays it in a web dashboard accessible from any browser.",
        es: "FitMesh Sync se conecta a Health Connect directamente, agrega datos de Galaxy Watch, Garmin y otros wearables sin duplicados, y los muestra en un panel web accesible desde cualquier navegador.",
        de: "FitMesh Sync verbindet sich direkt mit Health Connect, aggregiert Daten von Galaxy Watch, Garmin und anderen Wearables ohne Duplikate und zeigt sie in einem Dashboard an, das von jedem Browser aus zugänglich ist.",
        pt: "FitMesh Sync se conecta ao Health Connect diretamente, agrega dados do Galaxy Watch, Garmin e outros wearables sem duplicatas e os exibe em um painel web acessível de qualquer navegador.",
        fr: "FitMesh Sync se connecte directement à Health Connect, agrège les données de Galaxy Watch, Garmin et d'autres wearables sans doublons, et les affiche dans un tableau de bord web accessible depuis n'importe quel navigateur.",
        pl: "FitMesh Sync łączy się bezpośrednio z Health Connect, agreguje dane z Galaxy Watch, Garmin i innych wearable'ów bez duplikatów i wyświetla je w dashboardzie webowym dostępnym z dowolnej przeglądarki.",
        tr: "FitMesh Sync, Health Connect'e doğrudan bağlanır, Galaxy Watch, Garmin ve diğer giyilebilir cihazlardan verileri yinelemesiz toplar ve herhangi bir tarayıcıdan erişilebilen bir web gösterge panelinde gösterir.",
        nl: "FitMesh Sync verbindt rechtstreeks met Health Connect, aggregeert gegevens van Galaxy Watch, Garmin en andere wearables zonder duplicaten, en toont ze in een webdashboard dat toegankelijk is vanuit elke browser.",
        ja: "FitMesh SyncはHealth Connectに直接接続し、Galaxy Watch、Garmin、その他のウェアラブルからデータを重複なく集約して、どのブラウザからもアクセスできるWebダッシュボードに表示します。",
        ko: "FitMesh Sync는 Health Connect에 직접 연결하고, Galaxy Watch, Garmin 및 기타 웨어러블에서 데이터를 중복 없이 집계하여 모든 브라우저에서 접근 가능한 웹 대시보드에 표시합니다.",
      },
      ctaLabel: {
