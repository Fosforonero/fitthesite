...
export const post: BlogPost = {
  ...
  tldr: {
    it: [
      ...
      "Unisce marche diverse in una dashboard deduplicata: via Health Connect su Android, Apple Salute su iPhone (disponibile negli store supportati fuori dall'UE, rollout UE in corso), e l'anello Colmi via Bluetooth.",
    ...
    en: [
      ...
      "It merges different brands into one deduplicated dashboard: via Health Connect on Android, Apple Health on iPhone (available in supported storefronts outside the EU, EU rollout in progress), and the Colmi ring over Bluetooth.",
    ...
    es: [
      ...
      "Une marcas distintas en un único panel sin duplicados: vía Health Connect en Android, Apple Health en iPhone (disponible en las tiendas compatibles fuera de la UE, despliegue en la UE en curso) y el anillo Colmi por Bluetooth.",
    ...
    de: [
      ...
      "Es führt verschiedene Marken in einem Dashboard ohne Dubletten zusammen: über Health Connect unter Android, Apple Health auf dem iPhone (in unterstützten Stores außerhalb der EU verfügbar, EU-Rollout läuft) und den Colmi-Ring über Bluetooth.",
    ...
    pt: [
      ...
      "Ele reúne marcas diferentes em um único painel sem duplicatas: via Health Connect no Android, Apple Health no iPhone (disponível nas lojas compatíveis fora da UE, lançamento na UE em andamento) e o anel Colmi por Bluetooth.",
    ...
    fr: [
      ...
      "Il réunit des marques différentes dans un seul tableau de bord dédoublonné : via Health Connect sur Android, Apple Health sur iPhone (disponible dans les stores pris en charge hors de l'UE, déploiement dans l'UE en cours) et la bague Colmi par Bluetooth.",
  ...
  body: [
    ...
    },
    {
      ...
      body: {
        it: "FitMesh Sync è disponibile ora su Android e su iPhone (live sull'App Store negli store supportati fuori dall'Unione Europea; il rollout nei 27 paesi UE è in corso), con dashboard web inclusa. I primi 1000 iscritti founder ottengono il Pro a vita gratis; dopo, c'è una prova completa di 14 giorni, poi lo tieni con un piccolo abbonamento o con lo sblocco a vita (circa un caffè ogni sei mesi, o meno di una pizza). Scaricala oggi e inizia a costruire uno storico che sopravvive al prossimo cambio di orologio.",
        en: "FitMesh Sync is available now on Android and on iPhone (live on the App Store in supported storefronts outside the EU; rollout to the 27 EU countries is in progress), with the web dashboard included. The first 1,000 founder sign-ups get lifetime Pro free; after that there is a full 14-day trial, then you keep it with a small subscription or a one-time lifetime unlock (about a coffee every six months, or less than a pizza). Download it today and start building a history that survives your next change of watch.",
        es: "FitMesh Sync ya está disponible en Android y en iPhone (activo en la App Store en las tiendas compatibles fuera de la UE; el despliegue en los 27 países de la UE está en curso), con el panel web incluido. Los primeros 1000 registrados founder consiguen el Pro de por vida gratis; después hay una prueba completa de 14 días, y luego lo mantienes con una pequeña suscripción o con el desbloqueo de por vida (como un café cada seis meses, o menos que una pizza). Descárgala hoy y empieza a construir un historial que sobreviva a tu próximo cambio de reloj.",
        de: "FitMesh Sync ist ab sofort für Android und für iPhone verfügbar (im App Store in unterstützten Stores außerhalb der EU live; der Rollout in den 27 EU-Ländern läuft), inklusive Web-Dashboard. Die ersten 1.000 founder-Anmeldungen erhalten Pro lebenslang gratis; danach gibt es eine volle 14-tägige Testphase, und anschließend behältst du es mit einem kleinen Abo oder der einmaligen Freischaltung fürs Leben (weniger als ein Kaffee alle sechs Monate oder weniger als eine Pizza). Lade sie dir noch heute herunter und beginne einen Verlauf aufzubauen, der deinen nächsten Uhrenwechsel übersteht.",
        pt: "O FitMesh Sync já está disponível no Android e no iPhone (ativo na App Store nas lojas compatíveis fora da UE; o lançamento nos 27 países da UE está em andamento), com o painel web incluído. Os primeiros 1000 inscritos founder ganham o Pro vitalício de graça; depois, há uma avaliação completa de 14 dias e, em seguida, você o mantém com uma pequena assinatura ou com o desbloqueio vitalício (como um café a cada seis meses, ou menos que uma pizza). Baixe-o hoje e comece a construir um histórico que sobrevive à sua próxima troca de relógio.",
        fr: "FitMesh Sync est disponible dès maintenant sur Android et sur iPhone (actif sur l'App Store dans les stores pris en charge hors de l'UE ; déploiement dans les 27 pays de l'UE en cours), avec le tableau de bord web inclus. Les 1000 premiers inscrits founder obtiennent le Pro à vie gratuitement ; ensuite, il y a un essai complet de 14 jours, puis vous le gardez avec un petit abonnement ou un déblocage à vie en un seul paiement (moins qu'un café tous les six mois, ou moins qu'une pizza). Téléchargez-la dès aujourd'hui et commencez à bâtir un historique qui survit à votre prochain changement de montre.",
      },
      ctaLabel: {
