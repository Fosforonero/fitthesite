...
export const post: BlogPost = {
  ...
  tldr: {
    ...
    nl: [
      ...
      "Eenmaal ingesteld zijn je FitMesh-accountgegevens ook zichtbaar op iPhone (iOS-app binnenkort beschikbaar) en kunnen ze doorstromen naar Apple Health.",
    ...
    ja: [
      ...
      "設定完了後は、FitMesh アカウントのデータを iPhone でも確認できます（iOS アプリ近日公開）。Apple ヘルスにも連携可能です。",
    ...
    ko: [
      ...
      "설정 완료 후 FitMesh 계정 데이터는 iPhone에서도 확인할 수 있으며(iOS 앱 곧 출시), Apple 건강 앱으로도 연동됩니다.",
  ...
  body: [
    ...
    },
    {
      ...
      text: {
        ...
        nl: "De Bluetooth-verbinding tussen FitMesh Sync en de Colmi ring verloopt via Android, waar de data wordt verzameld en geüpload naar je FitMesh-account. Zodra de data in je account staat, kun je er overal bij: de FitMesh-app voor iPhone (iOS) en de Apple Health-brug komen binnenkort.",
        ja: "FitMesh Sync と Colmi リングの Bluetooth 接続は Android 上で行われ、データが収集されて FitMesh アカウントにアップロードされます。データがアカウントに入ると、どこからでもアクセスできます。iPhone 用 FitMesh アプリ（iOS）と Apple ヘルス連携は近日公開予定です。",
        ko: "FitMesh Sync와 Colmi 링 사이의 Bluetooth 연결은 Android에서 이루어지며, 데이터가 수집되어 FitMesh 계정에 업로드됩니다. 데이터가 계정에 들어오면 어디서든 접근할 수 있습니다. iPhone용 FitMesh 앱（iOS）과 Apple Health 브리지는 곧 출시됩니다.",
      },
    },
    {
      type: "callout",
      ...
      title: {
        ...
        nl: "Gebruik op iPhone (iOS-app binnenkort beschikbaar)",
        ja: "iPhone での使用（iOS アプリ近日公開）",
        ko: "iPhone에서 사용（iOS 앱 곧 출시）",
      },
      body: {
        ...
        nl: "Als je een iPhone gebruikt, stel de Colmi ring in via FitMesh Sync op een Android-apparaat (van jezelf of een familielid). De data wordt geüpload naar je FitMesh-account en is beschikbaar via de iOS-app zodra die uitkomt. Bovendien laat de optionele Apple Health-brug ringmeetwaarden (slaap, hartslag, SpO2) direct doorstromen naar de Apple Gezondheid-app. De FitMesh Sync iOS-app is nog niet beschikbaar in de App Store: meld je aan voor de beta om als een van de eersten toegang te krijgen.",
        ja: "iPhone を使っている場合は、Android デバイス（自分のもの、または家族のもの）で FitMesh Sync を通じて Colmi リングを設定してください。データは FitMesh アカウントにアップロードされ、iOS アプリが利用可能になり次第アクセスできます。さらに、オプションの Apple ヘルス連携により、リングのメトリクス（睡眠、心拍数、SpO2）が直接 Apple のヘルスアプリに流れ込みます。FitMesh Sync の iOS アプリはまだ App Store で利用できません。ベータ版に登録して、最初にアクセスできるユーザーの一人になりましょう。",
        ko: "iPhone을 사용하는 경우, Android 기기（자신의 것 또는 가족의 것）에서 FitMesh Sync를 통해 Colmi 링을 설정하세요. 데이터는 FitMesh 계정에 업로드되며 iOS 앱이 출시되는 즉시 이용할 수 있습니다. 또한 선택적 Apple Health 브리지를 통해 링 측정값（수면, 심박수, SpO2）이 Apple 건강 앱으로 직접 전달됩니다. FitMesh Sync iOS 앱은 아직 App Store에서 이용할 수 없습니다. 베타에 등록하여 가장 먼저 이용해 보세요.",
      },
    },
  ...
  faq: [
    ...
    },
    {
      ...
      a: {
        ...
        nl: "De Bluetooth-verbinding tussen FitMesh Sync en de Colmi ring vereist Android. Eenmaal ingesteld op Android worden de data echter geüpload naar je FitMesh-account en zijn ze ook op iPhone te bekijken via de FitMesh iOS-app (binnenkort beschikbaar) en optioneel in Apple Health. Als je alleen een iPhone hebt, meld je dan aan voor de beta: je wordt op de hoogte gesteld bij de lancering van de iOS-app.",
        ja: "FitMesh Sync と Colmi リングの Bluetooth 接続には Android が必要です。ただし、Android で設定した後、データは FitMesh アカウントにアップロードされ、FitMesh iOS アプリ（近日公開）を通じて iPhone でも表示でき、オプションで Apple ヘルスにも連携できます。iPhone しかお持ちでない場合は、ベータ版にご登録ください。iOS アプリのリリース時に通知が届きます。",
        ko: "FitMesh Sync와 Colmi 링 사이의 Bluetooth 연결에는 Android가 필요합니다. 그러나 Android에서 설정 완료 후 데이터는 FitMesh 계정에 업로드되며, FitMesh iOS 앱（곧 출시）을 통해 iPhone에서도 볼 수 있고 선택적으로 Apple Health에도 연동됩니다. iPhone만 갖고 있다면 베타에 등록하세요. iOS 앱 출시 시 알림을 받게 됩니다.",
      },
    },
    {
      q: {
