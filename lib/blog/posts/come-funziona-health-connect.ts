...
export const post: BlogPost = {
  ...
  tldr: {
    it: [
      ...
      "L'integrazione Garmin su Android dipende dall'app Garmin Connect o da API dedicate: non tutte le metriche passano automaticamente a Health Connect.",
      "Health Connect consente alle nuove app di leggere di norma fino a 30 giorni precedenti alla prima autorizzazione (se già scritti dalla sorgente); l'accesso a dati più vecchi richiede permessi storici aggiuntivi.",
    ...
    en: [
      ...
      "Garmin integration on Android depends on the Garmin Connect companion app or dedicated APIs: not all metrics sync automatically to Health Connect.",
      "Health Connect typically allows apps to read up to 30 days of data prior to initial authorization (if written by the source); older historical data requires specific permissions.",
    ...
    es: [
      ...
      "La integración de Garmin en Android depende de la app Garmin Connect o de APIs dedicadas: no todas las métricas pasan automáticamente a Health Connect.",
      "Por lo general, Health Connect permite leer hasta 30 días previos a la autorización inicial (si la fuente los escribió); los datos más antiguos requieren permisos históricos específicos.",
    ...
    de: [
      ...
      "Die Garmin-Integration unter Android hängt von der Garmin Connect App oder separaten APIs ab: Nicht alle Messwerte werden automatisch an Health Connect übertragen.",
      "Health Connect erlaubt neu autorisierten Apps standardmäßig das Lesen von Daten bis zu 30 Tage vor der ersten Freigabe (sofern von der Quelle hinterlegt); ältere Verlaufsdaten erfordern gesonderte Berechtigungen.",
    ...
    pt: [
      ...
      "A integração do Garmin no Android depende do app Garmin Connect ou de APIs dedicadas: nem todas as métricas são enviadas automaticamente ao Health Connect.",
      "O Health Connect geralmente permite a leitura de até 30 dias anteriores à autorização inicial (se gravados pela fonte); dados mais antigos exigem permissões específicas.",
    ...
    fr: [
      ...
      "L'intégration de Garmin sur Android dépend de l'application Garmin Connect ou d'API dédiées : toutes les métriques ne sont pas automatiquement transférées vers Health Connect.",
      "Health Connect permet généralement de lire jusqu'à 30 jours de données précédant la première autorisation (si enregistrées par la source) ; l'accès à un historique plus ancien nécessite des autorisations spécifiques.",
    ...
    pl: [
      ...
      "Integracja Garmin w systemie Android zależy od aplikacji Garmin Connect lub dedykowanych API: nie wszystkie parametry trafiają automatycznie do Health Connect.",
      "Health Connect pozwala zazwyczaj odczytać dane z maksymalnie 30 dni przed pierwszą autoryzacją (jeśli zostały zapisane przez źródło); starsze dane wymagają specjalnych uprawnień historycznych.",
    ...
    tr: [
      ...
      "Android'de Garmin entegrasyonu Garmin Connect uygulamasına veya özel API'lere bağlıdır: tüm ölçümler otomatik olarak Health Connect'e aktarılmaz.",
      "Health Connect, yeni uygulamaların genellikle ilk yetkilendirmeden önceki 30 güne kadar olan verileri okumasına izin verir (kaynak tarafından yazılmışsa); daha eski veriler özel geçmiş izinleri gerektirir.",
    ...
    nl: [
      ...
      "Garmin-integratie op Android is afhankelijk van de Garmin Connect-app of speciale API's: niet alle meetwaarden stromen automatisch naar Health Connect.",
      "Health Connect staat nieuwe apps doorgaans toe om gegevens tot 30 dagen vóór de eerste autorisatie te lezen (indien geschreven door de bron); oudere geschiedenis vereist specifieke machtigingen.",
    ...
    ja: [
      ...
      "AndroidでのGarmin連携はGarmin Connectアプリや個別APIに依存しており、すべての指標が自動的にHealth Connectへ同期されるわけではありません。",
      "Health Connectは通常、初回権限付与前の最大30日間のデータを読み取ることができます（送信元アプリが書き込んでいる場合）。それ以前の過去データには専用の履歴権限が必要です。",
    ...
    ko: [
      ...
      "Android에서 Garmin 연동은 Garmin Connect 앱 또는 별도 API에 의존하며, 모든 지표가 Health Connect로 자동 전송되지는 않습니다.",
      "Health Connect는 일반적으로 최초 권한 부여 전 최대 30일까지의 데이터를 읽을 수 있습니다(소스 앱이 기록한 경우). 그 이전의 과거 데이터에는 별도의 기록 권한이 필요합니다.",
  ...
  body: [
    ...
    },
    {
      ...
      items: {
        it: [
          "**Finestra storica predefinita a 30 giorni**: per impostazione predefinita, Health Connect consente alle nuove app di leggere fino a 30 giorni di storico precedenti alla prima concessione, a patto che l'app companion li abbia registrati. Dati antecedenti richiedono permessi storici specifici.",
          ...
          "**Integrazione variabile tra i produttori**: brand come Garmin o Polar dipendono dalla configurazione specifica della propria app companion o da API esterne, e alcuni tipi di dato potrebbero non essere esportati su Health Connect.",
        ...
        en: [
          "**Default 30-day historical window**: By default, Health Connect allows newly connected apps to read up to 30 days of historical data prior to first permission grant, provided the companion app wrote it. Accessing records older than 30 days requires dedicated historical permissions.",
          ...
          "**Variable manufacturer support**: Ecosystems like Garmin or Polar depend on specific companion app sync settings or external APIs, and certain metrics may not export to Health Connect.",
        ...
        es: [
          "**Ventana histórica estándar de 30 días**: por defecto, Health Connect permite que las nuevas apps lean hasta 30 días anteriores a la autorización, siempre que la app companion los haya registrado. Los datos más antiguos requieren un permiso histórico dedicado.",
          ...
          "**Soporte variable según el fabricante**: marcas como Garmin o Polar dependen de la configuración de su app companion o de APIs externas, y ciertas métricas no se exportan a Health Connect.",
        ...
        de: [
          "**Standardmäßiges 30-Tage-Verlaufsfenster**: Health Connect erlaubt neuen Apps standardmäßig das Auslesen von Verlaufsdaten der letzten 30 Tage vor der ersten Berechtigung, sofern die Companion-App diese gespeichert hat. Für ältere Daten ist eine spezielle Berechtigung erforderlich.",
          ...
          "**Unterschiedliche Herstellerunterstützung**: Marken wie Garmin oder Polar hängen von spezifischen Einstellungen ihrer Companion-Apps oder externen Schnittstellen ab; manche Metriken werden nicht an Health Connect übergeben.",
        ...
        pt: [
          "**Janela histórica padrão de 30 dias**: por padrão, o Health Connect permite que novos apps leiam até 30 dias de dados anteriores à primeira concessão, desde que o app companion os tenha gravado. Dados anteriores exigem permissão de histórico dedicada.",
          ...
          "**Suporte variável por fabricante**: marcas como Garmin ou Polar dependem da configuração do próprio app companion ou de APIs externas, e certas métricas não são exportadas ao Health Connect.",
        ...
        fr: [
          "**Fenêtre d'historique standard de 30 jours**: par défaut, Health Connect autorise les nouvelles applications à lire jusqu'à 30 jours précédant la première autorisation, sous réserve que l'application compagnon les ait enregistrés. Les données antérieures nécessitent une autorisation d'historique dédiée.",
          ...
          "**Prise en charge variable selon les marques**: Garmin ou Polar dépendent des réglages de leur application compagnon ou d'API externes, et certaines métriques ne sont pas exportées vers Health Connect.",
        ...
        pl: [
          "**Domyślne 30-dniowe okno historii**: domyślnie Health Connect umożliwia nowym aplikacjom odczyt danych z 30 dni przed pierwszą autoryzacją, o ile aplikacja towarzysząca je zarejestrowała. Dostęp do starszych danych wymaga dedykowanego uprawnienia historycznego.",
          ...
          "**Zmienne wsparcie producentów**: marki takie jak Garmin czy Polar zależą od konfiguracji aplikacji towarzyszącej lub zewnętrznych API, a niektóre dane mogą nie trafiać do Health Connect.",
        ...
        tr: [
          "**Varsayılan 30 günlük geçmiş penceresi**: Health Connect, companion uygulamanın yazmış olması kaydıyla, yeni uygulamaların ilk yetkiden önceki 30 güne kadar verileri okumasına olanak tanır. Daha eski veriler özel geçmiş izni gerektirir.",
          ...
          "**Farklı üretici desteği**: Garmin veya Polar gibi markalar kendi companion uygulamalarının ayarlarına veya harici API'lere dayanır ve bazı veriler Health Connect'e aktarılmayabilir.",
        ...
        nl: [
          "**Standaard historisch venster van 30 dagen**: standaard kunnen nieuwe apps gegevens lezen tot 30 dagen vóór de eerste toestemming, mits de companion-app deze heeft geregistreerd. Voor oudere gegevens is een specifieke geschiedenismachtiging vereist.",
          ...
          "**Wisselende fabrikantenondersteuning**: merken zoals Garmin of Polar zijn afhankelijk van companion-app-instellingen of externe API's; sommige metingen worden niet naar Health Connect geëxporteerd.",
        ...
        ja: [
          "**過去30日間の標準履歴枠**：コンパニオンアプリが書き込みを行っていれば、新しいアプリは初回承認前30日までのデータを読み取ることができます。30日を超える過去データへのアクセスには専用の履歴アクセス権限が必要です。",
          ...
          "**メーカーごとの連携差異**：GarminやPolarなどはコンパニオンアプリの設定や外部APIに依存しており、一部のデータ項目はHealth Connectに出力されない場合があります。",
        ...
        ko: [
          "**기본 30일 기록 기간**: 컴패니언 앱이 기록한 경우, 새 앱은 최초 권한 부여 전 최대 30일까지의 과거 데이터를 읽을 수 있습니다. 30일 이전의 데이터에는 전용 과거 기록 권한이 필요합니다.",
          ...
          "**제조사별 연동 차이**: Garmin이나 Polar 등의 브랜드는 컴패니언 앱 설정이나 외부 API에 의존하며, 일부 지표는 Health Connect로 내보내지지 않을 수 있습니다.",
    ...
    },
    {
      ...
      items: {
        it: [
          ...
          "**Dati antecedenti a 30 giorni**: l'accesso standard allo storico copre al massimo i 30 giorni precedenti alla prima autorizzazione, a condizione che l'app sorgente li abbia effettivamente trasferiti a Health Connect.",
        ],
        en: [
          ...
          "**Data older than 30 days**: Standard historical access is limited to a maximum of 30 days prior to the first authorization, provided the source app previously wrote those records to Health Connect.",
        ],
        es: [
          ...
          "**Datos anteriores a 30 días**: el acceso estándar al historial cubre como máximo los 30 días previos a la primera autorización, siempre que la app de origen los haya transferido a Health Connect.",
        ],
        de: [
          ...
          "**Daten vor dem 30-Tage-Fenster**: Der Standardzugriff auf Verlaufsdaten ist auf maximal 30 Tage vor der Erstautorisierung beschränkt, vorausgesetzt die Ursprungs-App hat diese an Health Connect übertragen.",
        ],
        pt: [
          ...
          "**Dados anteriores a 30 dias**: o acesso padrão ao histórico cobre no máximo 30 dias antes da primeira autorização, desde que o app de origem os tenha transferido ao Health Connect.",
        ],
        fr: [
          ...
          "**Données antérieures à 30 jours**: l'accès standard à l'historique est limité à 30 jours précédant la première autorisation, à condition que l'application source les ait bien transférées à Health Connect.",
        ],
        pl: [
          ...
          "**Dane starsze niż 30 dni**: standardowy dostęp do historii obejmuje maksymalnie 30 dni przed pierwszą autoryzacją, pod warunkiem że aplikacja źródłowa przekazała je wcześniej do Health Connect.",
        ],
        tr: [
          ...
          "**30 günden eski veriler**: standart geçmiş erişimi, kaynak uygulamanın bu verileri önceden Health Connect'e yazmış olması şartıyla ilk yetkilendirmeden önceki en fazla 30 günü kapsar.",
        ],
        nl: [
          ...
          "**Gegevens ouder dan 30 dagen**: standaardhistorietoegang dekt maximaal 30 dagen vóór de eerste autorisatie, mits de bron-app deze eerder naar Health Connect heeft geschreven.",
        ],
        ja: [
          ...
          "**30日以上前の過去データ**：標準の履歴読み取りは初回承認前の最大30日間に限定されます（送信元アプリがHealth Connectに記録済みの場合に限る）。",
        ],
        ko: [
          ...
          "**30일 이전의 과거 데이터**: 표준 기록 접근은 소스 앱이 Health Connect에 사전 기록한 경우에 한해 최초 승인 전 최대 30일로 제한됩니다.",
        ],
      },
    },
    {
    ...
    },
    {
      ...
      items: {
        it: [
          ...
          "Il flusso tipico è: sensori Watch → app companion (Samsung Health, Fitbit) → Health Connect → app terze. Ogni step richiede permessi espliciti.",
          "L'integrazione Garmin su Android dipende dall'app Garmin Connect o da API dedicate: non tutte le metriche passano automaticamente a Health Connect.",
          "L'accesso allo storico è limitato per impostazione predefinita ai 30 giorni precedenti alla prima concessione, salvo specifici permessi storici aggiuntivi.",
          "Il problema di sync più comune non è un bug hardware: è un permesso mancante in Health Connect. Verifica lì per primo.",
        ],
        en: [
          ...
          "The typical flow is: Watch sensors → companion app (Samsung Health, Fitbit) → Health Connect → third-party apps. Each step requires explicit permissions.",
          "Garmin integration on Android depends on the Garmin Connect companion app or dedicated APIs: not all metrics sync automatically to Health Connect.",
          "Historical access is limited by default to 30 days prior to initial authorization, unless specialized historical permissions are granted.",
          "The most common sync problem isn't a hardware bug: it's a missing permission in Health Connect. Check there first.",
        ],
        es: [
          ...
          "El flujo típico es: sensores del reloj → app companion (Samsung Health, Fitbit) → Health Connect → apps de terceros. Cada paso requiere permisos explícitos.",
          "La integración de Garmin en Android depende de la app Garmin Connect o de APIs dedicadas: no todas las métricas pasan automáticamente a Health Connect.",
          "El acceso al historial está limitado por defecto a los 30 días anteriores a la primera autorización, salvo permisos históricos adicionales.",
          "El problema de sincronización más frecuente no es un fallo de hardware: es un permiso faltante en Health Connect. Compruébalo ahí primero.",
        ],
        de: [
          ...
          "Der typische Datenfluss ist: Watch-Sensoren → Companion-App (Samsung Health, Fitbit) → Health Connect → Drittanbieter-Apps. Jeder Schritt erfordert explizite Berechtigungen.",
          "Die Garmin-Integration unter Android hängt von der Garmin Connect App oder separaten APIs ab: Nicht alle Messwerte werden automatisch an Health Connect übertragen.",
          "Der Zugriff auf Verlaufsdaten ist standardmäßig auf 30 Tage vor der ersten Freigabe beschränkt, es sei denn, es liegen zusätzliche historische Berechtigungen vor.",
          "Das häufigste Synchronisierungsproblem ist kein Hardware-Fehler: Es ist eine fehlende Berechtigung in Health Connect. Prüfe das zuerst.",
        ],
        pt: [
          ...
          "O fluxo típico é: sensores do smartwatch → app companion (Samsung Health, Fitbit) → Health Connect → apps de terceiros. Cada etapa requer permissões explícitas.",
          "A integração do Garmin no Android depende do app Garmin Connect ou de APIs dedicadas: nem todas as métricas são enviadas automaticamente ao Health Connect.",
          "O acesso ao histórico é limitado por padrão a 30 dias antes da primeira concessão, exceto com permissões de histórico adicionais.",
          "O problema de sincronização mais comum não é um bug de hardware: é uma permissão ausente no Health Connect. Verifique lá primeiro.",
        ],
        fr: [
          ...
          "Le flux typique est: capteurs de la montre connectée → application companion (Samsung Health, Fitbit) → Health Connect → applications tierces. Chaque étape nécessite des autorisations explicites.",
          "L'intégration de Garmin sur Android dépend de l'application Garmin Connect ou d'API dédiées : toutes les métriques ne sont pas automatiquement transférées vers Health Connect.",
          "L'accès aux données historiques est limité par défaut à 30 jours avant la première autorisation, sauf autorisations d'historique supplémentaires.",
          "Le problème de synchronisation le plus courant n'est pas un bug matériel: c'est une autorisation manquante dans Health Connect. Vérifiez-y en premier.",
        ],
        pl: [
          ...
          "Typowy przepływ to: sensory Watch → aplikacja companion (Samsung Health, Fitbit) → Health Connect → aplikacje zewnętrzne. Każdy krok wymaga wyraźnych uprawnień.",
          "Integracja Garmin w systemie Android zależy od aplikacji Garmin Connect lub dedykowanych API: nie wszystkie parametry trafiają automatycznie do Health Connect.",
          "Dostęp do danych historycznych jest domyślnie ograniczony do 30 dni przed pierwszą autoryzacją, chyba że przyznano dodatkowe uprawnienia historyczne.",
          "Najczęstszy problem z synchronizacją to nie błąd sprzętu: to brakujące uprawnienie w Health Connect. Sprawdź tam najpierw.",
        ],
        tr: [
          ...
          "Tipik akış şudur: Watch sensörleri → companion uygulama (Samsung Health, Fitbit) → Health Connect → üçüncü taraf uygulamalar. Her adım açık izin gerektirir.",
          "Android'de Garmin entegrasyonu Garmin Connect uygulamasına veya özel API'lere bağlıdır: tüm ölçümler otomatik olarak Health Connect'e aktarılmaz.",
          "Geçmiş verilere erişim, ek geçmiş izinleri verilmedikçe varsayılan olarak ilk yetkilendirmeden önceki 30 gün ile sınırlıdır.",
          "En yaygın senkronizasyon sorunu bir donanım hatası değildir: Health Connect'te eksik bir izindir. Önce oraya bakın.",
        ],
        nl: [
          ...
          "De typische stroom is: Watch-sensoren → companion-app (Samsung Health, Fitbit) → Health Connect → apps van derden. Elke stap vereist expliciete toestemmingen.",
          "Garmin-integratie op Android is afhankelijk van de Garmin Connect-app of speciale API's: niet alle meetwaarden stromen automatisch naar Health Connect.",
          "Toegang tot historische gegevens is standaard beperkt tot 30 dagen vóór de eerste autorisatie, tenzij aanvullende geschiedenismachtigingen zijn verleend.",
          "Het meest voorkomende synchronisatieprobleem is geen hardwarebug: het is een ontbrekende toestemming in Health Connect. Controleer dat eerst.",
        ],
        ja: [
          ...
          "AndroidでのGarmin連携はGarmin Connectアプリや個別APIに依存しており、すべての指標が自動的にHealth Connectへ同期されるわけではありません。",
          "過去データへのアクセスは、特別な履歴権限が付与されていない限り、初回承認前の30日間に制限されます。",
          "最も一般的な同期の問題はハードウェアのバグではありません。Health Connectで権限が不足していることが原因です。まずそこを確認してください。",
        ],
        ko: [
          ...
          "일반적인 흐름은 Watch 센서 → 컴패니언 앱(Samsung Health, Fitbit) → Health Connect → 서드파티 앱입니다. 각 단계에는 명시적 권한이 필요합니다.",
          "Android에서 Garmin 연동은 Garmin Connect 앱 또는 별도 API에 의존하며, 모든 지표가 Health Connect로 자동 전송되지는 않습니다.",
          "과거 데이터 접근은 추가 기록 권한이 부여되지 않는 한 최초 승인 전 30일로 기본 제한됩니다.",
          "가장 일반적인 동기화 문제는 하드웨어 버그가 아닙니다. Health Connect에서 권한이 누락된 것입니다. 먼저 그곳을 확인하세요.",
        ],
      },
    },
    {
      ...
      body: {
        it: "FitMesh Sync legge i dati da Health Connect e li mostra nell'app. Compatibile con Galaxy Watch, Pixel Watch, e altri wearable Android via Health Connect.",
        en: "FitMesh Sync reads data from Health Connect and shows it in the app. Compatible with Galaxy Watch, Pixel Watch, and other Android wearables via Health Connect.",
        es: "FitMesh Sync lee los datos desde Health Connect y los muestra en la app. Compatible con Galaxy Watch, Pixel Watch y otros wearables Android mediante Health Connect.",
        de: "FitMesh Sync liest Daten aus Health Connect und zeigt sie in der App an. Kompatibel mit Galaxy Watch, Pixel Watch und anderen Android-Wearables über Health Connect.",
        pt: "FitMesh Sync lê os dados do Health Connect e os exibe no app. Compatível com Galaxy Watch, Pixel Watch e outros wearables Android via Health Connect.",
        fr: "FitMesh Sync lit les données depuis Health Connect et les affiche dans l'application. Compatible avec Galaxy Watch, Pixel Watch et d'autres wearables Android via Health Connect.",
        pl: "FitMesh Sync odczytuje dane z Health Connect i wyświetla je w aplikacji. Kompatybilny z Galaxy Watch, Pixel Watch i innymi wearables Android przez Health Connect.",
        tr: "FitMesh Sync, Health Connect'ten veri okur ve bunları uygulamada gösterir. Galaxy Watch, Pixel Watch ve Health Connect aracılığıyla diğer Android wearable'larla uyumludur.",
        nl: "FitMesh Sync leest gegevens uit Health Connect en toont ze in de app. Compatibel met Galaxy Watch, Pixel Watch en andere Android-wearables via Health Connect.",
        ja: "FitMesh SyncはHealth Connectからデータを読み取り、アプリに表示します。Galaxy Watch、Pixel Watch、Health Connect経由のその他のAndroidウェアラブルに対応しています。",
        ko: "FitMesh Sync는 Health Connect에서 데이터를 읽어 앱에 표시합니다. Galaxy Watch, Pixel Watch, 그리고 Health Connect를 통한 다른 Android 웨어러블과 호환됩니다.",
      },
      ctaLabel: {
  ...
  faq: [
    ...
    },
    {
      ...
      a: {
        it: "Ci sono tre ragioni comuni. Prima: l'app non ha il permesso per quel tipo specifico di dato (controlla in Health Connect → Autorizzazioni app). Seconda: l'app companion non sta scrivendo su Health Connect per quel dato (verifica le impostazioni nell'app sorgente). Terza: i dati cercati risalgono a oltre 30 giorni prima della prima autorizzazione, oppure l'app sorgente non ha trasferito lo storico precedente nel database di Health Connect.",
        en: "There are three common reasons. First: the app lacks permission for that specific data type (check Health Connect → App permissions). Second: the companion app is not configured to write that metric to Health Connect. Third: the requested data is older than 30 days prior to your initial authorization, or the companion app did not transfer its past history into Health Connect.",
        es: "Hay tres razones frecuentes. Primera: la app no tiene permiso para ese dato específico. Segunda: la app companion no está configurada para escribir ese dato en Health Connect. Tercera: los datos buscados tienen más de 30 días de antigüedad respecto a la autorización inicial, o la app de origen no transfirió su historial previo a Health Connect.",
        de: "Es gibt drei häufige Gründe. Erstens: Der App fehlt die Berechtigung für diesen Datentyp. Zweitens: Die Companion-App überträgt diese Daten nicht nach Health Connect. Drittens: Die gesuchten Daten liegen mehr als 30 Tage vor der Erstautorisierung oder die Ursprungs-App hat ihren bisherigen Verlauf nicht in Health Connect hinterlegt.",
        pt: "Há três motivos comuns. Primeiro: o app não tem permissão para aquele tipo de dado. Segundo: o app companion não está configurado para gravar esse dado no Health Connect. Terceiro: os dados procurados têm mais de 30 dias em relação à primeira autorização ou o app de origem não transferiu seu histórico para o Health Connect.",
        fr: "Il y a trois causes fréquentes. Première : l'application n'a pas l'autorisation pour cette donnée. Deuxième : l'application compagnon n'est pas configurée pour écrire cette donnée dans Health Connect. Troisième : les données recherchées datent de plus de 30 jours avant la première autorisation, ou l'application d'origine n'a pas transféré son historique dans Health Connect.",
        pl: "Istnieją trzy powszechne powody. Po pierwsze: brak uprawnień do tego typu danych. Po drugie: aplikacja towarzysząca nie zapisuje tych danych w Health Connect. Po trzecie: poszukiwane dane pochodzą sprzed ponad 30 dni przed pierwszą autoryzacją lub aplikacja źródłowa nie zapisała wcześniejszej historii w Health Connect.",
        tr: "Üç yaygın neden vardır. Birincisi: uygulamanın o veri türü için izni yoktur. İkincisi: companion uygulama o veriyi Health Connect'e aktarmıyordur. Üçüncüsü: aranan veriler ilk yetkilendirmeden 30 günden daha eskiye aittir veya kaynak uygulama önceki geçmişini Health Connect'e aktarmamıştır.",
        nl: "Er zijn drie veelvoorkomende redenen. Ten eerste: de app heeft geen toestemming voor dat specifieke datatype. Ten tweede: de companion-app schrijft die meetwaarde niet naar Health Connect. Ten derde: de gezochte gegevens dateren van meer dan 30 dagen vóór uw eerste autorisatie of de bron-app heeft de eerdere geschiedenis niet overgezet naar Health Connect.",
        ja: "一般的な理由は3つあります。1つ目は、アプリに該当データタイプの読み取り権限がないこと。2つ目は、コンパニオンアプリがそのデータをHealth Connectに書き込む設定になっていないこと。3つ目は、対象データが初回承認より30日以上前のものであるか、送信元アプリが過去の履歴をHealth Connectに移行していないことです。",
        ko: "세 가지 흔한 이유가 있습니다. 첫째, 앱에 해당 데이터 유형에 대한 권한이 없습니다. 둘째, 컴패니언 앱이 해당 지표를 Health Connect에 쓰도록 설정되지 않았습니다. 셋째, 찾으시는 데이터가 최초 권한 부여 30일 이전의 것이거나 소스 앱이 과거 기록을 Health Connect로 이전하지 않았습니다.",
      },
    },
  ],
  related: [
