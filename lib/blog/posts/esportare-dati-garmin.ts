  ...
  body: [
    ...
    },
    {
      ...
      body: {
        ...
        fr: "Parmi les options qui simplifient cela figure FitMesh Sync: il lit vos données Garmin Connect via Health Connect (Garmin Connect écrit dans Health Connect depuis 2024) et apporte vos séances dans l'application, sans avoir à exporter des fichiers manuellement à chaque fois.",
        pl: "Jedną z opcji, które to upraszczają, jest FitMesh Sync: odczytuje Twoje dane Garmin Connect przez Health Connect (Garmin Connect zapisuje dane w Health Connect od 2024 roku) i przynosi Twoje treningi do aplikacji, bez konieczności ręcznego eksportowania plików za każdym razem.",
        tr: "Bunu kolaylaştıran seçenekler arasında FitMesh Sync var: Garmin Connect verilerinizi Health Connect üzerinden okur (Garmin Connect 2024'ten beri Health Connect'e veri yazıyor) ve antrenmanlarınızı her seferinde manuel dosya dışa aktarmanıza gerek kalmadan uygulamaya getirir.",
        nl: "Een van de opties die dit vereenvoudigt is FitMesh Sync: het leest uw Garmin Connect-gegevens via Health Connect (Garmin Connect schrijft sinds 2024 naar Health Connect) en brengt uw trainingen naar de app, zonder elke keer bestanden handmatig te exporteren.",
        ja: "これを簡単にするオプションの一つが FitMesh Sync です。Health Connect経由でGarmin Connectのデータを読み取り（Garmin Connectは2024年からHealth Connectに書き込んでいます）、毎回手動でファイルをエクスポートすることなく、アプリにトレーニングを表示します。",
        ko: "이것을 간단하게 만드는 옵션 중 하나가 FitMesh Sync입니다. Health Connect를 통해 Garmin Connect 데이터를 읽어오며(Garmin Connect는 2024년부터 Health Connect에 데이터를 기록하고 있습니다), 매번 파일을 수동으로 내보내지 않고도 앱에 운동을 가져옵니다.",
      },
      ctaLabel: {
