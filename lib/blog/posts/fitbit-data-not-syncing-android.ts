...
export const post: BlogPost = {
  ...
  body: [
    ...
    },
    {
      type: "cta",
      title: {
        it: "Vuoi vedere i dati Fitbit uniti a quelli dei tuoi altri dispositivi in un'unica app, senza configurazioni complesse?",
        en: "Want to see your Fitbit data merged with your other devices in one app, without complex configuration?",
        es: "¿Quieres ver tus datos de Fitbit unidos a los de tus otros dispositivos en una sola app, sin configuraciones complejas?",
        de: "Möchtest du deine Fitbit-Daten zusammen mit denen deiner anderen Geräte in einer einzigen App sehen, ohne komplexe Konfiguration?",
        pt: "Quer ver seus dados do Fitbit unidos aos dos seus outros dispositivos em um único app, sem configurações complexas?",
        fr: "Vous souhaitez voir vos données Fitbit réunies à celles de vos autres appareils dans une seule application, sans configuration complexe ?",
        pl: "Chcesz zobaczyć dane Fitbit połączone z danymi innych urządzeń w jednej aplikacji, bez skomplikowanej konfiguracji?",
        tr: "Fitbit verilerinizi diğer cihazlarınızın verileriyle birleştirilmiş şekilde tek bir uygulamada, karmaşık yapılandırma olmadan görmek ister misiniz?",
        nl: "Wilt u uw Fitbit-gegevens samen met die van uw andere apparaten in één app zien, zonder complexe configuratie?",
        ja: "複雑な設定なしに、Fitbitのデータを他のデバイスのデータと合わせて一つのアプリで確認したいですか？",
        ko: "복잡한 설정 없이 Fitbit 데이터를 다른 기기 데이터와 함께 하나의 앱에서 보고 싶으신가요?",
      },
      body: {
        it: "FitMesh Sync si integra con Fitbit via la sua API ufficiale e porta passi, frequenza cardiaca, sonno e allenamenti nell'app FitMesh. Nessun problema di permessi Health Connect: la connessione Fitbit è diretta e guidata passo passo.",
        en: "FitMesh Sync integrates with Fitbit via its official API and brings steps, heart rate, sleep, and workouts into the FitMesh app. No Health Connect permission headaches: the Fitbit connection is direct and guided step by step.",
        es: "FitMesh Sync se integra con Fitbit a través de su API oficial y lleva pasos, frecuencia cardíaca, sueño y entrenamientos a la app FitMesh. Sin problemas de permisos en Health Connect: la conexión con Fitbit es directa y guiada paso a paso.",
        de: "FitMesh Sync verbindet sich mit Fitbit über die offizielle API und bringt Schritte, Herzfrequenz, Schlaf und Trainings in die FitMesh-App. Keine Probleme mit Health Connect-Berechtigungen: Die Fitbit-Verbindung ist direkt und wird Schritt für Schritt geführt.",
        pt: "FitMesh Sync integra com o Fitbit via API oficial e leva passos, frequência cardíaca, sono e treinos para o app FitMesh. Sem problemas de permissões do Health Connect: a conexão com o Fitbit é direta e guiada passo a passo.",
        fr: "FitMesh Sync s'intègre à Fitbit via son API officielle et apporte pas, fréquence cardiaque, sommeil et séances d'entraînement dans l'application FitMesh. Aucun problème d'autorisation Health Connect : la connexion Fitbit est directe et guidée étape par étape.",
        pl: "FitMesh Sync integruje się z Fitbit przez oficjalne API i przenosi kroki, tętno, sen i treningi do aplikacji FitMesh. Żadnych problemów z uprawnieniami Health Connect: połączenie z Fitbit jest bezpośrednie i prowadzone krok po kroku.",
        tr: "FitMesh Sync, Fitbit ile resmi API üzerinden entegre olur ve adımları, kalp atış hızını, uykuyu ve antrenmanları FitMesh uygulamasına taşır. Health Connect izin sorunları yok: Fitbit bağlantısı doğrudan ve adım adım rehberli.",
        nl: "FitMesh Sync integreert met Fitbit via de officiële API en brengt stappen, hartslag, slaap en trainingen naar de FitMesh-app. Geen gedoe met Health Connect-toestemmingen: de Fitbit-verbinding is direct en stap voor stap begeleid.",
        ja: "FitMesh Syncは公式APIを通じてFitbitと連携し、歩数、心拍数、睡眠、ワークアウトをFitMeshアプリに集約します。Health Connectの権限問題はありません：Fitbitの接続は直接的でステップバイステップでガイドされます。",
        ko: "FitMesh Sync는 공식 API를 통해 Fitbit과 연동되며 걸음 수, 심박수, 수면, 운동을 FitMesh 앱으로 가져옵니다. Health Connect 권한 문제 없음: Fitbit 연결은 직접적이고 단계별로 안내됩니다.",
      },
      ctaLabel: {
