...
export const post: BlogPost = {
  ...
  body: [
    ...
    },
    {
      ...
      text: {
        ...
        fr: "FitMesh Sync a été conçu exactement pour ce scénario: plusieurs wearables, données fragmentées, aucun tableau de bord qui les rassemble proprement. L'application lit depuis Health Connect et affiche vos données dans un panneau unique, résolvant les problèmes les plus courants pour les utilisateurs de plusieurs appareils.",
        pl: "FitMesh Sync powstał dokładnie z myślą o tym scenariuszu: wiele wearables, fragmentaryczne dane, brak panelu, który łączyłby je w czysty sposób. Aplikacja odczytuje dane z Health Connect i wyświetla je w jednym panelu, rozwiązując najczęstsze problemy osób używających wielu urządzeń.",
        tr: "FitMesh Sync tam olarak bu senaryo için oluşturuldu: birden fazla giyilebilir, parçalanmış veriler, hepsini düzenli bir şekilde bir araya getiren pano yok. Uygulama Health Connect'ten okur ve verilerinizi tek bir panelde gösterir, birden fazla cihaz kullananların en yaygın sorunlarını çözer.",
        nl: "FitMesh Sync is precies voor dit scenario gebouwd: meerdere wearables, gefragmenteerde gegevens, geen dashboard dat alles overzichtelijk samenbrengt. De app leest van Health Connect en toont je gegevens in één overzichtelijk paneel, waarmee de meest voorkomende problemen voor mensen met meerdere apparaten worden opgelost.",
        ja: "FitMesh Sync はまさにこのシナリオのために作られました。複数のウェアラブル、断片化したデータ、それらをきれいにまとめるダッシュボードがない状況です。アプリは Health Connect からデータを読み取り、ひとつのパネルにまとめて表示し、複数のデバイスを使う人の最も一般的な問題を解決します。",
        ko: "FitMesh Sync는 정확히 이 시나리오를 위해 만들어졌습니다. 여러 웨어러블, 분산된 데이터, 이를 깔끔하게 한데 모아주는 대시보드가 없는 상황입니다. 앱은 Health Connect에서 데이터를 읽어 하나의 패널에 표시하며, 여러 기기를 사용하는 사람들의 가장 흔한 문제를 해결합니다.",
      },
    },
    {
      type: "list",
    ...
    },
    {
      ...
      body: {
        it: "FitMesh Sync legge i tuoi dati da Health Connect e li mostra in un pannello unificato nell'app. Niente doppioni, fonte del dato visibile. Disponibile su Google Play.",
        en: "FitMesh Sync reads your data from Health Connect and shows it in a unified panel in the app. No duplicates, data source visible. Available on Google Play.",
        es: "FitMesh Sync lee tus datos de Health Connect y los muestra en un panel unificado en la app. Sin duplicados, fuente del dato visible. Disponible en Google Play.",
        de: "FitMesh Sync liest deine Daten aus Health Connect und zeigt sie in einem einheitlichen Panel in der App an. Keine Duplikate, Datenquelle sichtbar. Verfügbar auf Google Play.",
        pt: "O FitMesh Sync lê seus dados do Health Connect e os exibe em um painel unificado no app. Sem duplicatas, fonte do dado visível. Disponível no Google Play.",
  ...
  faq: [
    ...
    },
    {
      ...
      a: {
        it: "FitMesh Sync non è un sostituto diretto di Google Fit: non registra dati dai sensori né fa da app companion per i wearable. È un aggregatore che legge i dati già presenti in Health Connect (o nelle API Garmin, Polar, etc.) e li mostra in un pannello unificato nell'app, con deduplicazione e visibilità della fonte. Se cerchi qualcosa per unificare i dati di più wearable in un'unica vista, FitMesh Sync serve a quello.",
        en: "FitMesh Sync is not a direct replacement for Google Fit: it doesn't record data from sensors or act as a companion app for wearables. It's an aggregator that reads data already present in Health Connect (or in Garmin, Polar APIs, etc.) and shows it in a unified panel in the app, with deduplication and source visibility. If you're looking for something to unify data from multiple wearables in a single view, FitMesh Sync serves that purpose.",
        es: "FitMesh Sync no es un sustituto directo de Google Fit: no registra datos de sensores ni actúa como app complementaria para wearables. Es un agregador que lee los datos ya presentes en Health Connect (o en las API de Garmin, Polar, etc.) y los muestra en un panel unificado en la app, con deduplicación y visibilidad de la fuente. Si buscas algo para unificar los datos de varios wearables en una sola vista, FitMesh Sync sirve para eso.",
        de: "FitMesh Sync ist kein direkter Ersatz für Google Fit: Es zeichnet keine Daten von Sensoren auf und fungiert nicht als Companion-App für Wearables. Es ist ein Aggregator, der Daten liest, die bereits in Health Connect (oder in den Garmin-, Polar-APIs usw.) vorhanden sind, und sie in einem einheitlichen Panel in der App mit Deduplizierung und Quellensichtbarkeit anzeigt. Wenn du etwas suchst, um Daten von mehreren Wearables in einer einzigen Ansicht zu vereinen, erfüllt FitMesh Sync genau diesen Zweck.",
        pt: "O FitMesh Sync não é um substituto direto do Google Fit: ele não registra dados de sensores nem atua como app companion para wearables. É um agregador que lê os dados já presentes no Health Connect (ou nas APIs do Garmin, Polar etc.) e os exibe em um painel unificado no app, com deduplicação e visibilidade da fonte. Se você está procurando algo para unificar os dados de vários wearables em uma única visão, o FitMesh Sync serve exatamente para isso.",
        fr: "FitMesh Sync n'est pas un remplacement direct de Google Fit: il n'enregistre pas de données depuis des capteurs et ne sert pas d'application compagnon pour les wearables. C'est un agrégateur qui lit les données déjà présentes dans Health Connect (ou dans les API Garmin, Polar, etc.) et les affiche dans un panneau unifié dans l'application, avec déduplication et visibilité de la source. Si vous cherchez quelque chose pour unifier les données de plusieurs wearables dans une seule vue, FitMesh Sync est fait pour ça.",
        pl: "FitMesh Sync nie jest bezpośrednim zastępcem dla Google Fit: nie rejestruje danych ze sensorów ani nie działa jako aplikacja towarzysząca do wearable. Jest to agregator, który odczytuje istniejące dane w Health Connect (lub API Garmin, Polar itp.) i prezentuje je na unifikowanym panelu w aplikacji, z deduplikacją i widocznością źródła. Jeśli szukasz rozwiązania do scalenia danych z wielu wearable w jednym miejscu, FitMesh Sync jest tym, czego potrzebujesz.",
        // P1.8C: "KVKK" (legge turca privacy) sostituiva "Google Fit", vedi
        // ledger Fase 4.
        tr: "FitMesh Sync, Google Fit'in doğrudan bir yerine geçeni değildir: sensörlerden veri kaydetmez ve giyilebilir cihazlar için companion uygulama olarak çalışmaz. Health Connect'te (veya Garmin, Polar API'lerinde vb.) zaten mevcut olan verileri okuyan ve bunları tekilleştirme ve kaynak görünürlüğüyle uygulamada birleşik bir panelde gösteren bir toplayıcıdır. Birden fazla giyilebilir cihazın verilerini tek bir görünümde birleştirmek istiyorsanız, FitMesh Sync tam olarak bunu yapar.",
        nl: "FitMesh Sync is geen directe vervanging voor Google Fit: het legt geen sensorgegevens vast en fungeert niet als companion-app voor wearables. Het is een aggregator die gegevens leest die al aanwezig zijn in Health Connect (of in de Garmin-, Polar-API's, etc.) en ze weergeeft in een unified paneel in de app, met deduplicatie en zichtbaarheid van de bron. Als je zoekt naar iets om gegevens van meerdere wearables in een enkele weergave samen te voegen, is FitMesh Sync daar voor.",
        ja: "FitMesh Sync は Google Fit の直接の代替ではありません。センサーからデータを記録したり、ウェアラブルのコンパニオンアプリとして機能したりはしません。Health Connect（または Garmin、Polar の API など）にすでに存在するデータを読み取り、重複排除とソース表示機能を備えた、アプリ内の統合パネルに表示する集約ツールです。複数のウェアラブルのデータを1つのビューで統合したい場合、FitMesh Sync がその役割を担います。",
        ko: "FitMesh Sync는 Google Fit의 직접적인 대체물이 아닙니다. 센서에서 데이터를 기록하거나 웨어러블의 컴패니언 앱 역할을 하지 않습니다. Health Connect(또는 Garmin, Polar API 등)에 이미 있는 데이터를 읽어 중복 제거와 소스 표시 기능이 있는 앱 내 통합 패널에 표시하는 집계 도구입니다. 여러 웨어러블의 데이터를 하나의 뷰에서 통합하려는 경우, FitMesh Sync가 그 역할을 합니다.",
      },
    },
  ],
  related: [
