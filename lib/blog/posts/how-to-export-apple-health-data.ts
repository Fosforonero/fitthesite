...
export const post: BlogPost = {
  ...
  tldr: {
    it: [
      ...
      "La via più comoda è una dashboard web senza export manuale: serve un'app che legga da HealthKit e sincronizzi in background. FitMesh iOS è live sull'App Store negli store supportati fuori dall'Unione Europea; il rollout nell'UE è in corso.",
    ],
    en: [
      ...
      "The most convenient path is a web dashboard without manual exports: it needs an app that reads from HealthKit and syncs in the background. FitMesh iOS is live on the App Store in supported storefronts outside the European Union; the EU rollout is in progress.",
    ],
    es: [
      ...
      "La opción más cómoda es un panel web sin exportaciones manuales: necesita una app que lea de HealthKit y sincronice en segundo plano. FitMesh iOS ya está disponible en App Store en las tiendas compatibles fuera de la Unión Europea; el despliegue en la UE está en curso.",
    ],
    de: [
      ...
      "Der bequemste Weg ist ein Web-Dashboard ohne manuelle Exporte: dafür braucht man eine App, die HealthKit ausliest und im Hintergrund synchronisiert. FitMesh iOS ist in unterstützten Stores außerhalb der Europäischen Union im App Store live; der Rollout in der EU läuft bereits.",
    ],
    pt: [
      ...
      "O caminho mais prático é um painel web sem exportações manuais: é necessário um app que leia do HealthKit e sincronize em segundo plano. FitMesh iOS já está disponível na App Store nas lojas compatíveis fora da União Europeia; o lançamento na UE está em andamento.",
    ],
    fr: [
      ...
      "L'option la plus pratique est un tableau de bord web sans exports manuels: il faut une application qui lit depuis HealthKit et synchronise en arrière-plan. FitMesh iOS est disponible sur l'App Store dans les boutiques prises en charge en dehors de l'Union européenne; le déploiement dans l'UE est en cours.",
    ],
    pl: [
      ...
      "Najwygodniejsza droga to panel webowy bez ręcznego eksportu: potrzebna jest aplikacja, która odczytuje z HealthKit i synchronizuje w tle. FitMesh iOS jest już dostępny w App Store w obsługiwanych sklepach poza Unią Europejską; wdrożenie w UE jest w toku.",
    ],
    tr: [
      ...
      "En pratik yol, manuel dışa aktarma gerektirmeyen bir web panosu: HealthKit'ten okuyan ve arka planda senkronize eden bir uygulama gerekiyor. FitMesh iOS Avrupa Birliği dışındaki desteklenen mağazalarda App Store'da yayında; AB'deki kullanıma sunma süreci devam ediyor.",
    ],
    nl: [
      ...
      "De handigste manier is een webdashboard zonder handmatige exports: daarvoor is een app nodig die HealthKit uitleest en op de achtergrond synchroniseert. FitMesh iOS is live in de App Store in ondersteunde landen buiten de Europese Unie; de uitrol in de EU is bezig.",
    ],
    ja: [
      ...
      "最も便利な方法は手動エクスポート不要のWebダッシュボードです：HealthKitから読み取りバックグラウンドで同期するアプリが必要です。FitMesh iOSはEU域外の対応ストアでApp Store配信中です。EUでの展開は準備中です。",
    ],
    ko: [
      ...
      "가장 편리한 방법은 수동 내보내기가 필요 없는 웹 대시보드입니다: HealthKit에서 읽고 백그라운드에서 동기화하는 앱이 필요합니다. FitMesh iOS는 EU 외 지원되는 스토어의 App Store에서 이용 가능합니다. EU 출시는 진행 중입니다.",
    ],
  },
  primaryKeyword: {
  ...
  body: [
    ...
    },
    {
      ...
      level: 2,
      text: {
        it: "Metodo 3: Dashboard web: la via più comoda (ora anche su iOS)",
        en: "Method 3: Web dashboard, the most convenient way (now on iOS too)",
        es: "Método 3: Panel web, la opción más cómoda (ya también en iOS)",
        de: "Methode 3: Web-Dashboard, der bequemste Weg (jetzt auch für iOS)",
        pt: "Método 3: Painel web, o caminho mais prático (agora também no iOS)",
        fr: "Méthode 3: Tableau de bord web, l'option la plus pratique (désormais aussi sur iOS)",
        pl: "Metoda 3: Panel webowy, najwygodniejsza opcja (teraz też na iOS)",
        tr: "Yöntem 3: Web panosu, en pratik yol (artık iOS'ta da)",
        nl: "Methode 3: Webdashboard, de handigste manier (nu ook op iOS)",
        ja: "方法3：Webダッシュボード、最も便利な方法（iOSでも利用可能に）",
        ko: "방법 3: 웹 대시보드, 가장 편리한 방법 (iOS에서도 이용 가능)",
      },
    },
    {
      type: "paragraph",
    ...
    },
    {
      type: "paragraph",
      text: {
        it: "FitMesh Sync è live sull'App Store per iPhone: disponibile negli store supportati fuori dall'Unione Europea, con il rollout nell'UE in corso. L'app iPhone legge i dati da Apple Health (HealthKit), li sincronizza automaticamente e li mostra sulla stessa dashboard web che già usiamo per Android. Feature extra: il ponte di scrittura (opt-in) porta i dati dei wearable Android direttamente in Apple Salute, utile se hai sia un dispositivo Android che un iPhone.",
        en: "FitMesh Sync is live on the App Store for iPhone: available in supported storefronts outside the European Union, with the EU rollout in progress. The iPhone app reads data from Apple Health (HealthKit), syncs it automatically, and displays it on the same web dashboard already used for Android. Extra feature: the write bridge (opt-in) brings Android wearable data directly into Apple Health, useful if you have both an Android device and an iPhone.",
        es: "FitMesh Sync ya está disponible en App Store para iPhone: disponible en las tiendas compatibles fuera de la Unión Europea, con el despliegue en la UE en curso. La app para iPhone lee los datos de Apple Health (HealthKit), los sincroniza automáticamente y los muestra en el mismo panel web que ya usamos para Android. Función extra: el puente de escritura (opt-in) lleva los datos de los wearables Android directamente a Apple Salud, muy útil si tienes tanto un dispositivo Android como un iPhone.",
        de: "FitMesh Sync ist jetzt live im App Store für iPhone: in unterstützten Stores außerhalb der Europäischen Union verfügbar, der Rollout in der EU läuft bereits. Die iPhone-App liest Daten aus Apple Health (HealthKit), synchronisiert sie automatisch und zeigt sie auf demselben Web-Dashboard an, das bereits für Android genutzt wird. Extra-Feature: die optionale Schreibbrücke (opt-in) überträgt Daten von Android-Wearables direkt in Apple Gesundheit, praktisch wenn Sie sowohl ein Android-Gerät als auch ein iPhone haben.",
        pt: "FitMesh Sync já está disponível na App Store para iPhone: disponível nas lojas compatíveis fora da União Europeia, com o lançamento na UE em andamento. O app para iPhone lê dados do Apple Health (HealthKit), sincroniza-os automaticamente e os exibe no mesmo painel web já usado para Android. Recurso extra: a ponte de escrita (opt-in) leva os dados de wearables Android diretamente para o Apple Saúde, útil se você tem tanto um dispositivo Android quanto um iPhone.",
        fr: "FitMesh Sync est désormais disponible sur l'App Store pour iPhone : disponible dans les boutiques prises en charge en dehors de l'Union européenne, déploiement dans l'UE en cours. L'app iPhone lit les données depuis Apple Santé (HealthKit), les synchronise automatiquement et les affiche sur le même tableau de bord web déjà utilisé pour Android. Fonctionnalité supplémentaire : le pont d'écriture (opt-in) transfère les données des appareils connectés Android directement dans Apple Santé, utile si vous avez à la fois un appareil Android et un iPhone.",
        pl: "FitMesh Sync jest już dostępny w App Store na iPhone: dostępny w obsługiwanych sklepach poza Unią Europejską, a wdrożenie w UE jest w toku. Aplikacja na iPhone odczytuje dane z Apple Health (HealthKit), synchronizuje je automatycznie i wyświetla na tym samym panelu webowym, który już używamy dla Androida. Dodatkowa funkcja: opcjonalny most zapisu (opt-in) przenosi dane urządzeń Android bezpośrednio do Apple Zdrowia – przydatne, jeśli masz zarówno urządzenie Android, jak i iPhone.",
        tr: "FitMesh Sync artık iPhone için App Store'da yayında: Avrupa Birliği dışındaki desteklenen mağazalarda mevcut, AB'deki kullanıma sunma süreci devam ediyor. iPhone uygulaması Apple Sağlık'tan (HealthKit) veri okuyor, otomatik senkronize ediyor ve Android için zaten kullandığımız aynı web panosunda gösteriyor. Ekstra özellik: isteğe bağlı yazma köprüsü (opt-in), Android giyilebilir verilerini doğrudan Apple Sağlık'a aktarıyor – hem Android cihazınız hem iPhone'unuz varsa çok kullanışlı.",
        nl: "FitMesh Sync is nu live in de App Store voor iPhone: beschikbaar in ondersteunde landen buiten de Europese Unie, de uitrol in de EU is bezig. De iPhone-app leest gegevens van Apple Gezondheid (HealthKit), synchroniseert ze automatisch en toont ze op hetzelfde webdashboard dat al wordt gebruikt voor Android. Extra functie: de schrijfbrug (opt-in) brengt Android-wearablegegevens rechtstreeks in Apple Gezondheid, handig als je zowel een Android-apparaat als een iPhone hebt.",
        ja: "FitMesh SyncはiPhone向けにApp Storeで配信中です：EU域外の対応ストアで利用可能で、EUでの展開は準備中です。iPhoneアプリはApple Health（HealthKit）からデータを読み取り、自動的に同期し、Androidですでに使用しているのと同じWebダッシュボードに表示します。追加機能：書き込みブリッジ（オプトイン）はAndroidウェアラブルのデータをApple Healthに直接書き込みます。AndroidデバイスとiPhoneの両方をお持ちの方に便利です。",
        ko: "FitMesh Sync는 iPhone용 App Store에서 이용 가능합니다: EU 외 지원되는 국가에서 다운로드할 수 있으며, EU 출시는 진행 중입니다. iPhone 앱은 Apple Health(HealthKit)에서 데이터를 읽고, 자동으로 동기화하며, Android에서 이미 사용 중인 동일한 웹 대시보드에 표시합니다. 추가 기능: 쓰기 브리지(옵트인)는 Android 웨어러블 데이터를 Apple Health에 직접 기록합니다. Android 기기와 iPhone을 모두 가진 분들에게 유용합니다.",
      },
    },
    {
      type: "callout",
      ...
      body: {
        it: "Molti utenti usano un iPhone ma hanno un wearable Android (es. Galaxy Watch o un anello smart). Con FitMesh Sync Android puoi già sincronizzare quei dati su web. La versione iOS (ora live sull'App Store) aggiunge anche il ponte di scrittura opt-in: i dati del wearable Android vengono scritti direttamente in Apple Salute, così li vedi nell'app Salute di iPhone senza alcun export manuale.",
        en: "Many users have an iPhone but an Android wearable (e.g. Galaxy Watch or a smart ring). With FitMesh Sync Android you can already sync that data to the web. The iOS version (now live on the App Store) also adds the opt-in write bridge: Android wearable data gets written directly into Apple Health, so you see it in the iPhone Health app without any manual export.",
        es: "Muchos usuarios tienen iPhone pero llevan un wearable Android (p. ej., Galaxy Watch o un anillo inteligente). Con FitMesh Sync para Android ya puedes sincronizar esos datos en la web. La versión iOS (ya disponible en App Store) añade además el puente de escritura opt-in: los datos del wearable Android se escriben directamente en Apple Salud, de modo que los ves en la app Salud del iPhone sin ninguna exportación manual.",
        de: "Viele Nutzer verwenden ein iPhone, tragen aber ein Android-Wearable (z. B. Galaxy Watch oder einen Smart-Ring). Mit FitMesh Sync Android können Sie diese Daten bereits mit dem Web synchronisieren. Die iOS-Version (jetzt live im App Store) fügt außerdem die optionale Schreibbrücke hinzu: Android-Wearable-Daten werden direkt in Apple Gesundheit geschrieben, sodass Sie sie in der Gesundheits-App des iPhone sehen, ohne manuellen Export.",
        pt: "Muitos usuários têm iPhone mas usam um wearable Android (ex.: Galaxy Watch ou um anel inteligente). Com FitMesh Sync Android você já pode sincronizar esses dados na web. A versão iOS (já disponível na App Store) adiciona também a ponte de escrita opt-in: os dados do wearable Android são gravados diretamente no Apple Saúde, então você os vê no app Saúde do iPhone sem nenhuma exportação manual.",
        fr: "De nombreux utilisateurs ont un iPhone mais portent un appareil connecté Android (ex.: Galaxy Watch ou une bague connectée). Avec FitMesh Sync Android, vous pouvez déjà synchroniser ces données sur le web. La version iOS (désormais disponible sur l'App Store) ajoute également le pont d'écriture opt-in: les données du wearable Android sont écrites directement dans Apple Santé, de sorte que vous les voyez dans l'app Santé de l'iPhone sans aucun export manuel.",
        pl: "Wielu użytkowników ma iPhone, ale nosi urządzenie Android (np. Galaxy Watch lub smart ring). Z FitMesh Sync na Androida możesz już synchronizować te dane na web. Wersja iOS (już dostępna w App Store) dodaje też opcjonalny most zapisu opt-in: dane urządzenia Android są zapisywane bezpośrednio w Apple Zdrowiu, dzięki czemu widzisz je w aplikacji Zdrowie iPhone'a bez żadnego ręcznego eksportu.",
        tr: "Pek çok kullanıcının iPhone'u var ama Android giyilebiliri (örn. Galaxy Watch veya akıllı yüzük) kullanıyor. FitMesh Sync Android ile bu verileri zaten web'e senkronize edebilirsiniz. iOS sürümü (artık App Store'da yayında) isteğe bağlı yazma köprüsünü de ekliyor: Android giyilebilir verileri doğrudan Apple Sağlık'a yazılıyor, böylece iPhone Sağlık uygulamasında hiç manuel dışa aktarma yapmadan görüyorsunuz.",
        nl: "Veel gebruikers hebben een iPhone maar een Android-wearable (bijv. Galaxy Watch of een slimme ring). Met FitMesh Sync Android kun je die gegevens al synchroniseren naar het web. De iOS-versie (nu live in de App Store) voegt ook de opt-in schrijfbrug toe: Android-wearablegegevens worden rechtstreeks in Apple Gezondheid geschreven, zodat je ze ziet in de iPhone Gezondheidsapp zonder handmatige export.",
        ja: "多くのユーザーがiPhoneを持ちながらAndroidウェアラブル（例：Galaxy Watchやスマートリング）を使用しています。FitMesh Sync Androidでそのデータをすでにウェブに同期できます。iOSバージョン（現在App Storeで配信中）もオプトインの書き込みブリッジを追加します：AndroidウェアラブルのデータがApple Healthに直接書き込まれ、手動でエクスポートせずにiPhoneのヘルスケアアプリで確認できます。",
        ko: "많은 사용자들이 iPhone을 가지고 있지만 Android 웨어러블(예: Galaxy Watch 또는 스마트 링)을 사용합니다. FitMesh Sync Android로 이미 그 데이터를 웹에 동기화할 수 있습니다. iOS 버전(현재 App Store에서 이용 가능)은 옵트인 쓰기 브리지도 추가합니다: Android 웨어러블 데이터가 Apple Health에 직접 기록되어 수동 내보내기 없이 iPhone 건강 앱에서 볼 수 있습니다.",
      },
    },
    {
      type: "callout",
    ...
    },
    {
      ...
      items: {
        it: [
          ...
          "La via più comoda (dashboard web senza export manuale) richiede un'app che legga da HealthKit e sincronizzi in background: FitMesh iOS è live sull'App Store negli store supportati fuori dall'Unione Europea; il rollout nell'UE è in corso.",
        ],
        en: [
          ...
          "The most convenient path (web dashboard without manual exports) requires an app that reads from HealthKit and syncs in the background: FitMesh iOS is live on the App Store in supported storefronts outside the European Union; the EU rollout is in progress.",
        ],
        es: [
          ...
          "Para compartir datos específicos con un médico, usa la exportación por métrica individual directamente desde el gráfico en Salud: genera un CSV limpio.",
          "El archivo XML no está cifrado: trátalo como un documento médico sensible y no lo subas a servicios en la nube públicos.",
          "La opción más cómoda (panel web sin exportaciones manuales) requiere una app que lea de HealthKit y sincronice en segundo plano: FitMesh iOS ya está disponible en App Store en las tiendas compatibles fuera de la Unión Europea; el despliegue en la UE está en curso.",
        ],
        de: [
          ...
          "Der bequemste Weg (Web-Dashboard ohne manuelle Exporte) erfordert eine App, die HealthKit ausliest und im Hintergrund synchronisiert: FitMesh iOS ist in unterstützten Stores außerhalb der Europäischen Union im App Store live; der Rollout in der EU läuft bereits.",
        ],
        pt: [
          ...
          "O arquivo XML não é criptografado: trate-o como um documento de saúde sensível e não o carregue em serviços de nuvem públicos.",
          "O caminho mais prático (painel web sem exportações manuais) requer um app que leia do HealthKit e sincronize em segundo plano: FitMesh iOS já está disponível na App Store nas lojas compatíveis fora da União Europeia; o lançamento na UE está em andamento.",
        ],
        fr: [
          ...
          "Pour partager des données précises avec un médecin, utilisez l'export par métrique individuelle directement depuis le graphique dans Santé: il produit un CSV propre.",
          "Le fichier XML n'est pas chiffré: traitez-le comme un document de santé sensible et ne le téléchargez pas sur des services cloud publics.",
          "L'option la plus pratique (tableau de bord web sans exports manuels) nécessite une application qui lit depuis HealthKit et synchronise en arrière-plan: FitMesh iOS est disponible sur l'App Store dans les boutiques prises en charge en dehors de l'Union européenne; le déploiement dans l'UE est en cours.",
        ],
        pl: [
          ...
          "Najwygodniejsza droga (panel webowy bez ręcznego eksportu) wymaga aplikacji, która odczytuje z HealthKit i synchronizuje w tle: FitMesh iOS jest już dostępny w App Store w obsługiwanych sklepach poza Unią Europejską; wdrożenie w UE jest w toku.",
        ],
        tr: [
          ...
          "En pratik yol (manuel dışa aktarma gerektirmeyen web panosu) HealthKit'ten okuyan ve arka planda senkronize eden bir uygulama gerektiriyor: FitMesh iOS Avrupa Birliği dışındaki desteklenen mağazalarda App Store'da yayında; AB'deki kullanıma sunma süreci devam ediyor.",
        ],
        nl: [
          ...
          "Om specifieke gegevens met een arts te delen, gebruik de export per statistiek direct vanuit de grafiek in Gezondheid: geeft een overzichtelijk CSV-bestand.",
          "Het XML-bestand is niet versleuteld: behandel het als een gevoelig medisch document en upload het niet naar openbare clouddiensten.",
          "De handigste manier (webdashboard zonder handmatige exports) vereist een app die HealthKit uitleest en op de achtergrond synchroniseert: FitMesh iOS is live in de App Store in ondersteunde landen buiten de Europese Unie; de uitrol in de EU is bezig.",
        ],
        ja: [
          ...
          "特定のデータを医師と共有するには、ヘルスケアのグラフから指標ごとのエクスポートを使用してください：クリーンなCSVが生成されます。",
          "XMLファイルは暗号化されていません：機密性の高い医療文書として扱い、パブリッククラウドサービスにアップロードしないでください。",
          "最も便利な方法（手動エクスポート不要のWebダッシュボード）にはHealthKitから読み取りバックグラウンドで同期するアプリが必要です：FitMesh iOSはEU域外の対応ストアでApp Store配信中です。EUでの展開は準備中です。",
        ],
        ko: [
          ...
          "특정 데이터를 의사와 공유하려면 건강 앱의 차트에서 지표별 내보내기를 사용하세요: 깔끔한 CSV 파일이 생성됩니다.",
          "XML 파일은 암호화되어 있지 않습니다: 민감한 의료 문서처럼 취급하고 공개 클라우드 서비스에 업로드하지 마세요.",
          "가장 편리한 방법(수동 내보내기가 필요 없는 웹 대시보드)에는 HealthKit에서 읽고 백그라운드에서 동기화하는 앱이 필요합니다: FitMesh iOS는 EU 외 지원되는 스토어의 App Store에서 이용 가능합니다. EU 출시는 진행 중입니다.",
        ],
      },
    },
    {
      type: "cta",
      title: {
        it: "Dashboard web per iPhone: scarica FitMesh Sync per iOS",
        en: "Web dashboard for iPhone: download FitMesh Sync for iOS",
        es: "Panel web para iPhone: descarga FitMesh Sync para iOS",
        de: "Web-Dashboard für iPhone: FitMesh Sync für iOS herunterladen",
        pt: "Painel web para iPhone: baixe o FitMesh Sync para iOS",
        fr: "Tableau de bord web pour iPhone : téléchargez FitMesh Sync pour iOS",
        pl: "Panel webowy dla iPhone: pobierz FitMesh Sync na iOS",
        tr: "iPhone için web panosu: FitMesh Sync'i iOS için indirin",
        nl: "Webdashboard voor iPhone: download FitMesh Sync voor iOS",
        ja: "iPhoneのためのWebダッシュボード：iOS版FitMesh Syncをダウンロード",
        ko: "iPhone을 위한 웹 대시보드: iOS용 FitMesh Sync 다운로드",
      },
      body: {
        it: "FitMesh Sync è live sull'App Store per iPhone, disponibile negli store supportati fuori dall'Unione Europea (il rollout nell'UE è in corso). L'app legge i dati da Apple Health e li sincronizza automaticamente su web, senza export manuali. Bonus: il ponte di scrittura (opt-in) porta anche i dati dei wearable Android dentro Apple Salute.",
        en: "FitMesh Sync is live on the App Store for iPhone, available in supported storefronts outside the European Union (EU rollout in progress). The app reads data from Apple Health and syncs it automatically to the web, no manual exports needed. Bonus: the write bridge (opt-in) also brings Android wearable data into Apple Health.",
        es: "FitMesh Sync ya está disponible en App Store para iPhone, en las tiendas compatibles fuera de la Unión Europea (el despliegue en la UE está en curso). La app lee los datos de Apple Health y los sincroniza automáticamente en la web, sin exportaciones manuales. Bonus: el puente de escritura (opt-in) también lleva los datos de los wearables Android a Apple Salud.",
        de: "FitMesh Sync ist live im App Store für iPhone, in unterstützten Stores außerhalb der Europäischen Union verfügbar (der Rollout in der EU läuft bereits). Die App liest Daten aus Apple Health und synchronisiert sie automatisch mit dem Web, ohne manuelle Exporte. Bonus: die optionale Schreibbrücke überträgt auch Android-Wearable-Daten in Apple Gesundheit.",
        pt: "FitMesh Sync já está disponível na App Store para iPhone, nas lojas compatíveis fora da União Europeia (o lançamento na UE está em andamento). O app lê dados do Apple Health e os sincroniza automaticamente na web, sem exportações manuais. Bônus: a ponte de escrita (opt-in) também traz dados de wearables Android para o Apple Saúde.",
        fr: "FitMesh Sync est désormais disponible sur l'App Store pour iPhone, dans les boutiques prises en charge en dehors de l'Union européenne (déploiement dans l'UE en cours). L'application lit les données depuis Apple Santé et les synchronise automatiquement sur le web, sans exports manuels. Bonus: le pont d'écriture (opt-in) transfère également les données des appareils connectés Android dans Apple Santé.",
        pl: "FitMesh Sync jest już dostępny w App Store na iPhone, w obsługiwanych sklepach poza Unią Europejską (wdrożenie w UE jest w toku). Aplikacja odczytuje dane z Apple Health i synchronizuje je automatycznie na web, bez ręcznego eksportu. Bonus: opcjonalny most zapisu (opt-in) przenosi też dane urządzeń Android do Apple Zdrowia.",
        tr: "FitMesh Sync artık iPhone için App Store'da yayında, Avrupa Birliği dışındaki desteklenen mağazalarda mevcut (AB'deki kullanıma sunma süreci devam ediyor). Uygulama Apple Sağlık'tan veri okuyor ve otomatik olarak web'e senkronize ediyor, manuel dışa aktarma yok. Bonus: isteğe bağlı yazma köprüsü (opt-in) Android giyilebilir verilerini de Apple Sağlık'a aktarıyor.",
        nl: "FitMesh Sync is live in de App Store voor iPhone, beschikbaar in ondersteunde landen buiten de Europese Unie (de uitrol in de EU is bezig). De app leest gegevens van Apple Gezondheid en synchroniseert ze automatisch naar het web, zonder handmatige exports. Bonus: de schrijfbrug (opt-in) brengt ook Android-wearablegegevens in Apple Gezondheid.",
        ja: "FitMesh SyncはiPhone向けにApp Storeで配信中です。EU域外の対応ストアで利用可能です（EUでの展開は準備中）。アプリはApple Healthからデータを読み取り、手動エクスポート不要で自動的にウェブに同期します。ボーナス：書き込みブリッジ（オプトイン）もAndroidウェアラブルのデータをApple Healthに取り込みます。",
        ko: "FitMesh Sync는 iPhone용 App Store에서 이용 가능합니다. EU 외 지원되는 국가에서 다운로드할 수 있습니다(EU 출시는 진행 중). 앱은 Apple Health에서 데이터를 읽고 수동 내보내기 없이 자동으로 웹에 동기화합니다. 보너스: 쓰기 브리지(옵트인)도 Android 웨어러블 데이터를 Apple Health에 가져옵니다.",
      },
      ctaLabel: {
