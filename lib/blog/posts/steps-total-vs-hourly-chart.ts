...
export const post: BlogPost = {
  ...
  // mandato. GO PUBBLICAZIONE (10/09/2026): comportamento attribuito
  // esplicitamente "dalla versione 3.10.0" nei tre blocchi corretti, cosi'
  // chi usa ancora una versione precedente non si aspetta lo stesso
  // funzionamento. updatedAt impostato alla data reale di pubblicazione.
  ...
  tldr: {
    it: [
      ...
      "Dalla versione 3.10.0: quando il dettaglio orario è utilizzabile e proviene dalla stessa fonte del totale, FitMesh può mostrare anche una curva parziale, dichiarandone i limiti. Se la provenienza non è verificabile, è diversa o i dati sono incoerenti, FitMesh non mostra quella curva anche se i dati orari esistono: non promette una copertura oraria completa. Su versioni precedenti, un disallineamento del genere nascondeva l'intero grafico senza questa distinzione.",
    ...
    en: [
      ...
      "Since version 3.10.0: when hourly detail is usable and comes from the same source as the total, FitMesh can show even a partial curve, stating its limits. If the provenance isn't verifiable, comes from elsewhere, or the numbers don't add up, FitMesh won't show that curve even though the hourly data exists: it doesn't promise full hourly coverage. On earlier versions, a mismatch like this hid the whole chart without this distinction.",
    ...
    de: [
      ...
      "Seit Version 3.10.0: Wenn das stündliche Detail nutzbar ist und aus derselben Quelle wie der Gesamtwert stammt, kann FitMesh auch eine unvollständige Kurve zeigen — mit einem Hinweis auf ihre Grenzen. Ist die Herkunft nicht überprüfbar, stammt sie aus einer anderen Quelle, oder sind die Daten widersprüchlich, zeigt FitMesh diese Kurve nicht, auch wenn die stündlichen Daten vorhanden sind: Eine vollständige stündliche Abdeckung wird nicht versprochen. In früheren Versionen blendete eine solche Abweichung das gesamte Diagramm aus, ohne diese Unterscheidung.",
    ...
    fr: [
      ...
      "Depuis la version 3.10.0 : quand le détail horaire est exploitable et provient de la même source que le total, FitMesh peut afficher une courbe même partielle, en indiquant ses limites. Si la provenance n'est pas vérifiable, est différente, ou si les données sont incohérentes entre elles, FitMesh n'affiche pas cette courbe même si les données horaires existent : une couverture horaire complète n'est pas garantie. Sur les versions antérieures, un tel écart masquait tout le graphique sans cette distinction.",
  ...
  body: [
    ...
    },
    {
      type: "paragraph",
      text: {
        it: "Il grafico ora per ora e il totale del giorno possono arrivare da fonti diverse. Dalla versione 3.10.0, FitMesh disegna la curva solo quando il dettaglio orario è utilizzabile e dimostrabilmente della stessa fonte del totale: se copre l'intera giornata la mostra intera, se ne copre solo una parte la mostra dichiarandola parziale, con la quota senza orario indicata accanto. Se invece la provenienza del dettaglio orario non è verificabile, viene da una fonte diversa dal totale, oppure i numeri sono incoerenti fra loro, FitMesh non disegna quella curva sotto il totale — anche quando i dati orari esistono: la mostrerebbe come se fosse la forma di un numero che non è il suo. Il totale del giorno resta comunque visibile in ogni caso, ma il grafico orario non è garantito coprire sempre l'intera giornata. Su versioni precedenti alla 3.10.0, questa distinzione non esisteva: un disallineamento del genere nascondeva sempre l'intero grafico.",
        en: "The hour-by-hour chart and the day's total can come from different sources. Since version 3.10.0, FitMesh only draws the curve when the hourly detail is usable and demonstrably from the same source as the total: when that's the case, if it covers the whole day it shows it in full, and if it covers only part of the day it shows it labeled as partial, with the portion that has no hourly breakdown noted alongside it. But if the hourly detail's provenance isn't verifiable, comes from a source other than the total's, or the numbers don't add up, FitMesh won't draw that curve under the total — even when the hourly data exists: that would show the shape of a number that isn't its own. The day's total stays visible either way, but the hourly chart isn't guaranteed to always cover the full day. On versions before 3.10.0, this distinction didn't exist: a mismatch like this always hid the whole chart.",
        de: "Das Stunde-für-Stunde-Diagramm und das Tagestotal können aus unterschiedlichen Quellen stammen. Seit Version 3.10.0 zeichnet FitMesh die Kurve nur, wenn das stündliche Detail nutzbar und nachweislich aus derselben Quelle wie der Gesamtwert ist: In diesem Fall zeigt es sie vollständig, wenn sie den ganzen Tag abdeckt, und als unvollständig gekennzeichnet, wenn sie nur einen Teil abdeckt — mit dem Anteil ohne stündliche Aufschlüsselung daneben angegeben. Ist dagegen die Herkunft des stündlichen Details nicht überprüfbar, stammt sie aus einer anderen Quelle als der Gesamtwert, oder sind die Zahlen widersprüchlich, zeichnet FitMesh diese Kurve nicht unter dem Gesamtwert — selbst wenn die stündlichen Daten vorhanden sind: Es würde damit die Form einer Zahl zeigen, die nicht ihre eigene ist. Das Tagestotal bleibt in jedem Fall sichtbar, das Stundendiagramm deckt aber nicht garantiert immer den ganzen Tag ab. Auf Versionen vor 3.10.0 gab es diese Unterscheidung nicht: Eine solche Abweichung blendete immer das gesamte Diagramm aus.",
        fr: "Le graphique heure par heure et le total du jour peuvent provenir de sources différentes. Depuis la version 3.10.0, FitMesh ne trace la courbe que lorsque le détail horaire est exploitable et manifestement issu de la même source que le total : dans ce cas, s'il couvre toute la journée, il l'affiche en entier, et s'il n'en couvre qu'une partie, il l'affiche en la signalant comme partielle, avec la part sans détail horaire indiquée à côté. Si en revanche la provenance du détail horaire n'est pas vérifiable, provient d'une source différente du total, ou si les chiffres sont incohérents entre eux, FitMesh ne trace pas cette courbe sous le total — même quand les données horaires existent : ce serait montrer la forme d'un nombre qui n'est pas le sien. Le total du jour reste visible dans tous les cas, mais le graphique horaire n'est pas garanti de toujours couvrir la journée entière. Sur les versions antérieures à la 3.10.0, cette distinction n'existait pas : un tel écart masquait toujours tout le graphique.",
      },
    },
    {
      type: "paragraph",
  ...
  faq: [
    {
      ...
      a: {
        it: "Dalla versione 3.10.0, succede quando il dettaglio orario disponibile non può essere mostrato come rappresentazione affidabile del tuo totale del giorno: perché viene da una fonte diversa da quella del totale, perché la sua provenienza non è verificabile, oppure perché i numeri sono incoerenti tra loro. In tutti questi casi FitMesh non disegna quella curva, anche se i dati orari esistono: preferisce non mostrarla piuttosto che presentarla come se fosse il tuo giorno intero. Il totale resta comunque visibile, ma il grafico orario non è garantito essere sempre completo. Su versioni precedenti, lo stesso disallineamento nascondeva sempre l'intero grafico.",
        en: "Since version 3.10.0, this happens when the available hourly detail can't be shown as a reliable picture of your day's total: because it comes from a source other than the total's, because its provenance isn't verifiable, or because the numbers don't add up. In every one of these cases, FitMesh won't draw that curve, even though the hourly data exists: it would rather not show it than present it as if it were your whole day. The total stays visible regardless, but the hourly chart isn't guaranteed to always be complete. On earlier versions, the same mismatch always hid the whole chart.",
        de: "Seit Version 3.10.0 passiert das, wenn die verfügbaren stündlichen Daten nicht als verlässliche Darstellung deines Tagestotals gezeigt werden können: weil sie aus einer anderen Quelle als das Total stammen, weil ihre Herkunft nicht überprüfbar ist, oder weil die Zahlen untereinander widersprüchlich sind. In all diesen Fällen zeichnet FitMesh diese Kurve nicht, auch wenn die stündlichen Daten vorhanden sind: Es zeigt sie lieber gar nicht, als sie so darzustellen, als wäre es dein ganzer Tag. Das Total bleibt trotzdem sichtbar, das Stundendiagramm ist aber nicht garantiert immer vollständig. Auf früheren Versionen blendete dieselbe Abweichung immer das gesamte Diagramm aus.",
        fr: "Depuis la version 3.10.0, cela arrive quand le détail horaire disponible ne peut pas être présenté comme une représentation fiable de votre total du jour : parce qu'il provient d'une source différente de celle du total, parce que sa provenance n'est pas vérifiable, ou parce que les chiffres sont incohérents entre eux. Dans tous ces cas, FitMesh préfère ne pas tracer cette courbe plutôt que de la montrer comme si elle représentait toute votre journée : le total reste visible malgré tout, mais le graphique horaire n'est pas garanti d'être toujours complet. Sur les versions antérieures, le même écart masquait toujours tout le graphique.",
      },
    },
    {
      q: {
