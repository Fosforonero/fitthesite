...
export const FITNESS_DATA_SYNC_FAQ: FaqEntry[] = [
  {
    ...
    a: {
      it: "Il fitness data sync significa spostare o combinare i dati di salute e allenamento tra le app e i wearable che usi davvero, così non devi controllare cinque app diverse per vedere passi, sonno e allenamenti. Può significare un health hub di piattaforma (Health Connect su Android, Apple Health su iOS), una bridge app che sposta dati da un'app specifica a un'altra, o una dashboard indipendente che legge da più fonti e le mostra in un unico posto. FitMesh Sync è il terzo tipo: una dashboard nella propria app.",
      en: "Fitness data sync means moving or combining health and workout data across the apps and wearables you actually use, so you don't have to check five different apps to see your steps, sleep and workouts. It can mean a platform-level health hub (Health Connect on Android, Apple Health on iOS), a bridge app that moves data from one specific app to another, or an independent dashboard that reads from multiple sources and shows them in one place. FitMesh Sync is the third kind: a dashboard in its own app.",
      es: "La sincronización de datos de fitness significa mover o combinar datos de salud y entrenamiento entre las apps y wearables que realmente usas, para que no tengas que revisar cinco apps distintas para ver tus pasos, sueño y entrenamientos. Puede significar un hub de salud a nivel de sistema (Health Connect en Android, Apple Health en iOS), una bridge app que mueve datos de una app específica a otra, o un panel independiente que lee de varias fuentes y las muestra en un solo lugar. FitMesh Sync es el tercer tipo: un panel en su propia app.",
      de: "Fitnessdaten-Synchronisierung bedeutet, Gesundheits- und Trainingsdaten über die Apps und Wearables zu verschieben oder zu kombinieren, die du tatsächlich nutzt, damit du nicht fünf verschiedene Apps prüfen musst, um deine Schritte, deinen Schlaf und deine Workouts zu sehen. Das kann ein plattformweiter Health-Hub sein (Health Connect auf Android, Apple Health auf iOS), eine Bridge-App, die Daten von einer bestimmten App zu einer anderen verschiebt, oder ein unabhängiges Dashboard, das aus mehreren Quellen liest und sie an einem Ort zeigt. FitMesh Sync ist die dritte Art: ein Dashboard in der eigenen App.",
    },
  },
  {
    q: {
