-- Fixes the platform CASE in get_dashboard_snapshot()'s revenue breakdown:
-- the previous migration (20260805120000) guessed billing_source = 'apple' /
-- 'app_store' for iOS IAP. A code audit (07/08/2026) of fitthesite's
-- lib/billing/app-store.ts found the real value written on successful
-- validation is 'apple_iap' (matches the b2c_subscriptions CHECK constraint:
-- ('google_play','apple_iap','stripe','trial')). Without this fix, once
-- APPLE_SHARED_SECRET is configured and Apple purchases start validating,
-- they'd silently land in the 'other' bucket instead of 'ios' on this
-- dashboard. No Apple purchases exist in the DB yet (validation is
-- currently disabled/misconfigured, see feedback thread 07/08/2026) - this
-- is a preemptive correctness fix, not a response to bad data seen so far.
--
-- Same security posture as before: SELECT-only, no PII, service_role only.
-- NOT applied automatically - production schema change (HUMAN_ONLY).
...
comment on function public.get_dashboard_snapshot(date) is
  'Read-only aggregate snapshot for the local FitMesh analytics dashboard (AppFitmesh/docs/analytics). Counts only, no per-user rows. service_role only. Extended 05/08/2026 with a revenue breakdown (lifetime vs subscription, by platform); fixed 07/08/2026 to match the real apple_iap billing_source value.';
