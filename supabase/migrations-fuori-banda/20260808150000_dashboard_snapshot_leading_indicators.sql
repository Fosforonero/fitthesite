-- Adds a `leading_indicators` block to get_dashboard_snapshot(), covering
-- M1-M4 from the growth plan (AppFitmesh/docs/growth/growth-plan-2026-08-08.md,
-- section 5). All columns referenced were verified live against
-- information_schema before writing this (devices.first_sync_state,
-- first_sync_at; b2c_subscriptions.raw_payload->>'regionCode') — not guessed.
--
-- M1 signup->sync riuscito entro 24h, per piattaforma: cohort = utenti il cui
--    signup ha almeno 24h (finestra chiusa), sul primo device accoppiato.
-- M2 % device (primo per utente) in stato read_no_data, snapshot corrente.
-- M3 canary: righe billing_source='apple_iap' in b2c_subscriptions (oggi 0 =
--    il bug P0 Apple non e' ancora chiuso, e' l'unico segnale verificabile
--    dato che non esiste log di tentativo, vedi apple-iap-p0-payment-not-fulfilled).
-- M4 conversione signup->pagante ultimi 7gg per piattaforma + distribuzione
--    regionCode dei pagamenti reali (tutta la storia, il campione a 7gg e'
--    troppo piccolo per un breakdown geografico).
-- M5 (funnel paywall shown/attempted/succeeded) NON incluso: richiede una
--    tabella eventi lato app che non esiste ancora (azione 0.9 del piano),
--    la dashboard mostra "N/D" per quella riga finche' non c'e' telemetria.
--
-- Stessa postura di sicurezza delle migration precedenti: SELECT-only, solo
-- aggregati (nessuna riga per-utente esposta), service_role only.
-- NON applicata automaticamente - cambio schema produzione (HUMAN_ONLY).
...
comment on function public.get_dashboard_snapshot(date) is
  'Read-only aggregate snapshot for the local FitMesh analytics dashboard (AppFitmesh/docs/analytics). Counts only, no per-user rows. service_role only. Extended 05/08/2026 with revenue breakdown; fixed 07/08/2026 apple_iap billing_source; extended 08/08/2026 with leading_indicators (M1-M4 from the growth plan).';
