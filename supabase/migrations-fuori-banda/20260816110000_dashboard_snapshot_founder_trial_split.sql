-- Aggiunge a get_dashboard_snapshot() lo split founder/non-founder tra gli
-- attivi ultimi 7gg, e lo split trial-ancora-attivo/trial-gia-scaduto tra chi
-- non ha nessun titolo (ne' founder, ne' altro grant pro, ne' pagamento
-- reale). Richiesta di Matteo 16/08/2026: quanti degli attivi vivono ancora
-- di founder gratuito, e quanti dei 14 giorni di prova sono gia' scaduti
-- senza conversione (il "muro" di cui si e' parlato per la coorte
-- post-cutoff 01/08).
--
-- Nessuna tabella nuova: estende activity_base (gia' usata per active_7d/30d)
-- con tre flag di titolarita' derivati da tabelle gia' lette altrove nella
-- stessa funzione (user_roles, b2c_subscriptions).
