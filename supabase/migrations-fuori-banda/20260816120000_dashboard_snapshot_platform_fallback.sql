-- Aggiunge un fallback alla classificazione piattaforma: se os_version non
-- risolve a iOS/Android (oggi 4 utenti, os_version null perché non hanno mai
-- completato un sync), prova first_sync_platform prima di arrendersi a
-- "unknown". first_sync_platform viene scritto da
-- record_first_sync_transition() quando il device sincronizza (con qualunque
-- esito, non solo successo) — quindi un utente oggi "non rilevato" che in
-- futuro sincronizza anche solo una volta puo' auto-correggersi al prossimo
-- refresh della dashboard, senza bisogno di un'altra migration. Richiesta di
-- Matteo 16/08/2026.
--
-- Verificato prima di scrivere: oggi first_sync_platform e' popolato per 248
-- device su ~582 (75 ios + 173 android, 334 null) - non e' un campo morto,
-- ma per i 4 utenti "unknown" specifici e' ancora null anche lui (nessuna
-- sincronizzazione mai avvenuta). Il fix quindi non cambia unknown_total
-- oggi (rimane 4, verificato), ma lo rende auto-correttivo da qui in avanti.
