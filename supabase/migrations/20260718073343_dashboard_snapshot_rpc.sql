-- Read-only aggregate snapshot for the local FitMesh analytics dashboard
-- (AppFitmesh/docs/analytics/dashboard-locale.html). Returns ONLY counts/aggregates,
-- never per-user rows, emails, or PII. EXECUTE is granted to service_role only —
-- called exclusively by tools/analytics-dashboard/refresh_dashboard.py with the
-- service-role key, never by anon/authenticated clients.
--
-- NOT applied automatically. Review before running via Supabase MCP apply_migration
-- or `supabase db push` — this is a production schema change (HUMAN_ONLY per
-- AppFitmesh/CLAUDE.md's "Aggiunta nuove tabelle Supabase (migrations 009+)" rule).
...
comment on function public.get_dashboard_snapshot(date) is
  'Read-only aggregate snapshot for the local FitMesh analytics dashboard (AppFitmesh/docs/analytics). Counts only, no per-user rows. service_role only.';
