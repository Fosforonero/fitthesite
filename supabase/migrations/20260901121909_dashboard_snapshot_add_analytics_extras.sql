-- Estende get_dashboard_snapshot() con quattro blocchi aggiuntivi (chiave
-- 'extras'), tutti aggregati/anonimi (nessun user_id, nessuna riga per
-- persona) per restare coerenti col resto della funzione: distribuzione
-- giorni-alla-conversione, divario di sync per settimana di pairing,
-- matrice di retention per coorte settimanale, riattivazione dopo 14gg+.
-- Additivo: nessuna chiave esistente tocca, nessun consumatore rompe.
