-- ============================================================================
-- ROLLBACK di 20260901121909 — get_dashboard_snapshot() torna alla definizione
-- precedente, quella di 20260818084202 (dashboard_snapshot_payments_history).
--
-- La migration NON crea oggetti: sostituisce il corpo di una funzione che
-- esiste da mesi. Un `drop` sarebbe quindi sbagliato — porterebbe via anche
-- cio' che c'era prima. Tornare indietro qui vuol dire RIMETTERE il corpo
-- precedente, ed e' l'unica forma corretta di rollback per un
-- `create or replace`.
--
-- Il testo qui sotto NON e' riscritto a mano: e' copiato alla lettera dalla
-- migration 20260818084202, estratto programmaticamente. Reinventarlo
-- sarebbe il modo piu' rapido per far divergere il rollback dallo stato che
-- dice di ripristinare.
--
-- Effetto: spariscono i quattro blocchi `extras` (churn, cohort_retention,
-- sync_gap_trend, time_to_convert_days). Nessuna chiave precedente cambia:
-- verificato che le 25 della vecchia definizione sono un sottoinsieme delle
-- 70 vive.
-- ============================================================================
