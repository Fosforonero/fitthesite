-- ============================================================================
-- ROLLBACK di 20260901151500_dashboard_snapshot_pro_attivi_con_extras
--
-- Torna alla definizione di 20260901121909 — quella VIVA in produzione dal
-- 01/09/2026 — e NON a quella di 20260818084202.
--
-- La differenza non e' una sottigliezza. 121909 e' cio' che il registro di
-- produzione contiene oggi, con il proprio `statements` byte per byte;
-- 20260818084202 e' due passi indietro e porterebbe via anche gli analytics
-- extras, che nessuno ha chiesto di togliere. Un rollback che scavalca uno
-- stato registrato non e' un rollback: e' una terza modifica.
--
-- Effetto: i quattro conteggi «Pro attivo» tornano a includere le righe
-- scadute e le due chiavi storiche spariscono. E' un ritorno a un difetto
-- NOTO e dichiarato, non a uno stato pulito: si esegue solo per riportare la
-- funzione allo stato del 01/09.
--
-- Il testo qui sotto NON e' riscritto a mano: e' copiato alla lettera dalla
-- migration 20260901121909, estratto programmaticamente. La postcondizione in
-- fondo lo verifica con l'md5 del corpo, che non e' un commento ma un
-- controllo: md5(prosrc) = f53b80ca007a26d944e5f93427cdab62.
-- ============================================================================
...
-- POSTCONDIZIONE
-- ============================================================================
-- Non «il file e' andato a buon fine», ma «la funzione viva e' esattamente
-- quella di 121909». L'md5 e' del corpo (prosrc), misurato sul file della
-- migration e confermato sulla definizione viva in produzione il 01/09/2026.
do $post$
declare
  v_md5 text;
begin
  ...
  if v_md5 is distinct from 'f53b80ca007a26d944e5f93427cdab62' then
    raise exception
      'ROLLBACK NON RIUSCITO: il corpo di get_dashboard_snapshot ha md5 %, '
      'atteso f53b80ca007a26d944e5f93427cdab62 (la definizione di 20260901121909). '
      'La funzione viva non e'' quella che questo rollback dice di aver ripristinato.',
      coalesce(v_md5, '<funzione assente>');
  end if;
  raise notice 'rollback: get_dashboard_snapshot e'' tornata a 20260901121909 (md5 verificato).';
end
$post$;
