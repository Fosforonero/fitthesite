...
psql_quiet() { docker exec -e PGPASSWORD=postgres "$CID" psql -U postgres -d "$DBN" -X -q -c "$1" >/dev/null; }
...
run_backfill() {  # $1 = "dry" | "apply"
  ...
  if [ "$1" = "apply" ]; then
    docker exec -e PGPASSWORD=postgres "$CID" \
      psql -U postgres -d "$DBN" -X -v ON_ERROR_STOP=1 -v claims_backfill_apply=1 \
      -f /tmp/bf_under_test.sql
  else
    docker exec -e PGPASSWORD=postgres "$CID" \
      psql -U postgres -d "$DBN" -X -v ON_ERROR_STOP=1 \
      -f /tmp/bf_under_test.sql
  fi
}
