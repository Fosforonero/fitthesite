...
for round in 1 2 3 4 5; do
  docker exec -e PGPASSWORD=postgres "$CID" psql -U postgres -d postgres -X -tA -c "$TX_A" > "$OUT1" 2>&1 &
  P1=$!
  docker exec -e PGPASSWORD=postgres "$CID" psql -U postgres -d postgres -X -tA -c "$TX_B" > "$OUT2" 2>&1 &
  P2=$!
  wait $P1 || true
  wait $P2 || true
